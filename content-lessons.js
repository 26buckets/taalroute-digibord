(function(root){
 ...
 function description(spec){try{spec=runtime.currentSelection(spec)}catch{}return spec.scope_clauses.map(c=>(c.topic_ids.length?c.topic_ids.map(labelTopic).join(', '):'Alle onderwerpen')+' · '+(c.cefr_levels.join(', ')||'Alle beschikbare niveaus')).join(' + ')}
 ...
 async function render(){
  ...
  try{
   ...
   const query=search.toLocaleLowerCase('nl').trim(),visible=(o,type)=>(!root.ReleasePolicy?.enabled||root.ReleasePolicy.selectionAllowed(o.selection_spec||o))&&(o.name+' '+description(o.selection_spec||o)).toLocaleLowerCase('nl').includes(query)&&(!onlyFavorites||fav(type,o[kindId(type)]));
   const cards=await Promise.all(lessons.filter(o=>visible(o,'saved_selection')).map(async o=>{const result=await service.resolveSavedSelection(o.saved_selection_id),ready=result.status==='READY';return `<article class="lesson-card"><span class="lesson-origin">${o.owner_scope==='system'?'Van Taalroute':o.owner_scope==='school'?'Van mijn school':'Mijn les'}</span><h3>${esc(o.name)}</h3><div class="lesson-scopes">${selectionLabels(o.selection_spec,o.name)}</div><p class="lesson-status">${ready?'Klaar':esc(result.message)}</p><div class="lesson-actions">${action('start',o.saved_selection_id,'Start',!ready)}${action('edit',o.saved_selection_id,'Aanpassen')}${toggle('saved_selection',o.saved_selection_id)}${action('archive',o.saved_selection_id,'Archiveren')}</div></article>`}));
   const recents=await Promise.all(recent.filter(o=>!root.ReleasePolicy?.enabled||root.ReleasePolicy.sessionAllowed(o.session_config_snapshot)).sort((a,b)=>b.last_active_at.localeCompare(a.last_active_at)).slice(0,12).map(async o=>{let canResume=false,reason='';try{await service.resumeRecentSession(o.recent_session_id);canResume=true}catch(error){reason=error.message}const session=o.session_config_snapshot;return `<article class="lesson-card"><h3>${esc(session.topic==='MIX'?'Mijn mix':labelTopic(session.topic))}</h3><div class="practice-labels">${(session.cefr_levels||[session.cefr_level]).map(ContentUI.levelBadge).join('')}</div><p>${esc(engines.get(session.selected_game_engine)?.label||'Spel')} · ${new Date(o.last_active_at).toLocaleDateString('nl-NL')}</p>${reason?`<p class="lesson-status">${esc(reason)}</p>`:''}<div class="lesson-actions">${canResume?action('resume',o.recent_session_id,'Ga verder'):''}${action('replay',o.recent_session_id,'Dezelfde opdrachten opnieuw')}${action('reroll',o.recent_session_id,'Nieuwe opdrachten')}${action('other',o.recent_session_id,'Ander spel')}${action('archive-recent',o.recent_session_id,'Archiveren')}</div></article>`}));
