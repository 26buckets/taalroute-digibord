...
function startBoard(board){
 ...
 <div class="board-option-row">${boardOptionTitle('modeSelect','Spelvorm','Klassikaal: de hele klas speelt met één pion. Individueel: iedere deelnemer heeft een eigen pion en beurt. Groepen: iedere groep speelt met één pion. Deelnemers en groepen stel je in via Menu → Instellingen.')}<select class="mode-select" id="modeSelect"><option value="individual">Individueel</option><option value="class">Klassikaal</option><option value="groups">Groepen</option></select></div>
...
function cardsFor(kind,all=false){if(kind==='c1-between-lines')return RUNTIME.c1BetweenLines.cards.filter(c=>all||!APP.c1Domain||c.domainId===Number(APP.c1Domain));if(kind==='tongue'){const bank=RUNTIME.tongueBank;return bank.cards.filter(c=>c.type==='tongbreker'&&(all||(bank.levels.indexOf(c.entryLevel)<=bank.levels.indexOf(tongueLevel())&&(!APP.tongueDifficulty||({easy:c.difficulty<=2,medium:c.difficulty===3,hard:c.difficulty>=4}[APP.tongueDifficulty]??true)))))}const list=CARD_GAMES.find(x=>x.id===kind)?.cards||[];if(kind==='idioms'&&!all)return list.filter(c=>(mixLevel()==='all'||c.targetLevel===mixLevel())&&((APP.mixCollection||'mix')==='all'||(APP.mixCollection==='legacy'?!c.taalmix:!!c.taalmix))&&(!APP.mixGroup||c.taalmix?.group===APP.mixGroup)&&(!APP.mixKind||(c.taalmix?.kind||(c.actionType==='rebus'?'Rebus':c.actionType==='proverb'?'Spreekwoord':'Uitdrukking'))===APP.mixKind));return all||activeCardRoute()==='all'?list:list.filter(c=>c.routeId===activeCardRoute())}
function mixLevel(){return ['A1','A2','B1','B2','C1','C2','all'].includes(APP.mixLevel)?APP.mixLevel:['A1','A2','B1','B2'].includes(APP.level)?APP.level:'A1'}
function mixFilters(){return `<div class="taalmix-filters">${[
 ['mixCollection','Kaartenset',[['mix','Taalmix · 240 kaarten'],['legacy','Overige bestaande kaarten'],['all','Alles samen']],APP.mixCollection||'mix'],
 ['mixGroup','Opbouw',[['','Alles'],['Instapper','Instappers'],['Vervolg','Vervolg'],['Uitdaging','Uitdaging']],APP.mixGroup||''],
 ['mixKind','Soort',[['','Alles'],['Dagelijkse taal','Dagelijkse taal'],['Uitdrukking','Uitdrukkingen'],['Spreekwoord','Spreekwoorden'],['Rebus','Rebussen']],APP.mixKind||'']
 ].map(([id,label,options,value])=>`<label>${label}<select id="${id}">${options.map(([v,t])=>`<option value="${v}" ${v===value?'selected':''}>${t}</option>`).join('')}</select></label>`).join('')}<p>60 kaarten per niveau A1–B2 · 20 instappers, 20 vervolg, 20 uitdagingen. Taalniveau en puzzelbelasting staan los van elkaar.</p></div>`}
...
function c1Filters(){return `<div class="taalmix-filters"><label>Domein<select id="c1Domain"><option value="">Alle vijf domeinen</option>${RUNTIME.c1BetweenLines.domains.map(d=>`<option value="${d.id}" ${String(d.id)===APP.c1Domain?'selected':''}>${esc(d.title)}</option>`).join('')}</select></label><button class="smallbtn" id="c1Previous">← Vorige kaart</button><button class="smallbtn" id="c1ViewSet">Bekijk alle 50 kaarten</button></div>`}
...
function startC1(){
 ...
 renderCardTable(bank.id,counter,`<div class="card-ribbon" style="--ribbon:${bank.color}"><strong>C1 · Batch 001</strong><span class="card-counter">${counter}</span></div><div class="card-content c1-content" data-card-id="${esc(c.id)}"><h2>${esc(c.expression)}</h2><div class="card-situation"><div><strong>Situatie</strong><p>${esc(c.situation)}</p></div></div><div class="c1-choices" role="group" aria-labelledby="c1Question"><h3 id="c1Question">${esc(c.question)}</h3>${c.options.map(o=>`<button class="smallbtn" data-c1-choice="${o.id}" aria-pressed="${state.choice===o.id}" ${state.choice?'disabled':''}>${o.id}. ${esc(o.text)}</button>`).join('')}</div><section id="c1Feedback" role="status" aria-live="polite" aria-atomic="true">${c1Feedback(c,state.choice)}</section></div>`);
}
...
function cardActivityHeader(kind){const item=CARD_GAMES.find(x=>x.id===kind);if(kind==='tongue')return `<div class="card-activity-heading"><h1>Tongbrekers <span>${cardsFor(kind,true).length} kaarten</span></h1>${tongueFilters()}</div>`;return `<div class="card-activity-heading"><div><h1>${esc(item.title)} <span>${item.cards.length} kaarten</span></h1><p>${esc(item.intro)}</p>${kind==='idioms'?mixFilters():kind==='c1-between-lines'?c1Filters():''}</div></div>`}
...
function cardTools(){const c=currentCard();if(!c||APP.cardKind==='c1-between-lines')return '';const state=cardRound(c),practice=c.model.showWhen==='before_during_after_attempt';return contextTools('card',{
...
function renderCardTable(kind,counter,front){const item=collectionItems().find(x=>x.type==='cards'&&x.id===kind);$('#gameMount').innerHTML=`<div class="game-shell card-table-shell${kind==='tongue'?' tongue-table':kind==='c1-between-lines'?' c1-table':''}"><div class="game-work card-work">${cardActivityHeader(kind)}<div class="cards-stage">${cardDeck(kind,counter)}<div class="game-card-motion"><article class="active-card">${front}${kind==='tongue'?'':cardTools()}</article><div class="game-card-back card-back-design" style="--deck-color:${item.color}" aria-hidden="true">${cardBack(item.title,'Kaartspel')}</div></div>${cardTypeChoices(kind)}</div></div>${gameBar(`<button class="primary card-next-primary" id="primaryGame">${cardFan()}<span>${kind==='tongue'?'Volgende':'VOLGENDE KAART'}</span></button>`)}</div>`;bindCards(kind)}
function bindCards(kind){
 ...
 if(kind==='c1-between-lines'){
  ...
  $$('[data-c1-choice]').forEach(button=>button.onclick=()=>{
   ...
   $('#c1Feedback').innerHTML=c1Feedback(c,state.choice);
   $('#c1Feedback').scrollIntoView({block:'nearest'});
  });
  return;
 }
 if(kind==='idioms'){
  for(const key of ['mixCollection','mixGroup','mixKind'])$('#'+key).onchange=e=>{rememberAction('taalmixfilter kiezen');APP[key]=e.target.value;APP.cardIndex=0;delete APP.cardRound;save();startCards(kind)};
...
function startCards(kind){
 ...
 const list=cardsFor(kind);if(!list.length){if(kind!=='idioms')return toast('Deze route bevat geen kaarten.');APP.cardKind=kind;APP.cardIndex=0;delete APP.cardRound;setLast('card',family.title,{kind});return renderCardTable(kind,'0 kaarten','<div class="card-content"><h2>Geen kaarten bij deze filters</h2><p>Kies een ander niveau, een andere opbouw of een andere soort. De Taalmix bevat kaarten voor A1 tot en met B2.</p></div>')}APP.cardKind=kind;APP.cardIndex=((APP.cardIndex||0)%list.length+list.length)%list.length;
...
function collectionItems(){
 const cardItems=CARD_GAMES.map(({id,title,description,color,pilot})=>({id,type:'cards',title,description,symbol:'◌',color,meta:cardsFor(id,true).length+(id==='c1-between-lines'?' kaarten · C1 · 5 domeinen':id==='tongue'?' tongbrekers · A0–C2':id==='idioms'?' kaarten · 240 Taalmix + 22 bestaande':' kaarten · 7 taalroutes')}));
...
function cabinetSetInfo(type,id){
 ...
 const records=cardsFor(id,true);if(id==='c1-between-lines')return {...item,records,activity:item.title,format:'C1 · Batch 001',how:'Kies zelfstandig één van de drie antwoorden. Daarna zie je goed of fout, het juiste antwoord, uitleg en Let op.',samples:records};if(id==='tongue')return {...item,description:records.length+' tongbrekers',records,activity:item.title,format:'Tongbrekers',how:'',samples:records};return {...item,records,activity:item.title,format:id==='idioms'?'Taalvormen en beeldrebussen met hulp en voorbeeld':'Tekstkaarten met hulp en een voorbeeldantwoord',how:id==='idioms'?'240 volledig uitgewerkte kaarten: per niveau 40 taalvormen en 20 beeldrebussen. Kies niveau, opbouw en soort tijdens het spelen. Overige bestaande kaarten blijven apart beschikbaar. Docentbeoordeling en lesproef staan nog open.':CARD_GAMES.find(x=>x.id===id)?.pilot?'Speelbare proefset: geef eerst mondeling een eigen antwoord. Hulp ondersteunt; Oplossing of Voorbeeld geeft uitleg. Meer biedt een vervolgopdracht. Deze startinhoud is nog niet in lessen gevalideerd.':id==='mission'?'Je krijgt een praktische taalopdracht. Speel de situatie met een ander en bekijk zo nodig het voorbeeld.':'Gebruik de vraag of opdracht om een gesprek te beginnen. Vertel, vraag door en laat anderen reageren.',samples:records.slice(0,3)};
}
...
function syncLevelSelect(cards){const select=$('#levelSelect');select.disabled=cards&&APP.cardKind==='c1-between-lines';if(select.disabled){select.dataset.mix=select.dataset.tongue=select.dataset.routes='false';select.setAttribute('aria-label','Niveau Nederlands tussen de regels');select.innerHTML='<option value="C1">C1</option>';return}select.dataset.mix=String(cards&&APP.cardKind==='idioms');if(cards&&APP.cardKind==='idioms'){select.dataset.tongue='false';select.dataset.routes='false';select.setAttribute('aria-label','Taalniveau Taalmix');select.innerHTML=['A1','A2','B1','B2','C1','C2','all'].map(l=>`<option value="${l}">${l==='all'?'Alle niveaus':l}</option>`).join('');select.value=mixLevel();return}select.dataset.tongue=String(cards&&APP.cardKind==='tongue');if(cards&&APP.cardKind==='tongue'){select.dataset.routes='false';select.setAttribute('aria-label','Niveau tongbrekers');select.innerHTML=RUNTIME.tongueBank.levels.map(l=>`<option value="${l}">t/m ${l}</option>`).join('');select.value=tongueLevel();return}select.dataset.routes=String(cards);select.setAttribute('aria-label',cards?'Taalroute':'Niveau');select.innerHTML=cards?`<option value="all">Alle routes</option>${CARD_ROUTES.map(r=>`<option value="${r.id}">${esc(r.label)}</option>`).join('')}`:LEVELS.map(l=>`<option>${l}</option>`).join('');select.value=cards?activeCardRoute():APP.level}
syncLevelSelect(false);
$('#levelSelect').onchange=e=>{
 if(e.target.dataset.mix==='true'){rememberAction('taalmixniveau kiezen');APP.mixLevel=e.target.value;APP.cardIndex=0;delete APP.cardRound;save();startCards('idioms');return}
