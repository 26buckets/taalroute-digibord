...
function cardActivityHeader(kind){const item=CARD_GAMES.find(x=>x.id===kind);if(kind==='tongue')return `<div class="card-activity-heading"><h1>Tongbrekers <span>${cardsFor(kind,true).length} kaarten</span></h1>${tongueFilters()}</div>`;return `<div class="card-activity-heading"><div><h1>${esc(item.title)} <span>${item.cards.length} kaarten</span></h1><p>${esc(item.intro)}</p>${kind==='c1-between-lines'?c1Filters():''}</div></div>`}
