...
function cardActivityHeader(kind){const item=CARD_GAMES.find(x=>x.id===kind);if(kind==='tongue')return `<div class="card-activity-heading"><h1>Tongbrekers <span>${cardsFor(kind,true).length} kaarten</span></h1>${tongueFilters()}</div>`;return `<div class="card-activity-heading"><div><h1>${esc(item.title)} <span>${item.cards.length} kaarten</span></h1><p>${esc(item.intro)}</p>${kind==='idioms'?mixFilters():''}</div></div>`}
...
function cardTools(){const c=currentCard();if(!c)return '';const state=cardRound(c),practice=c.model.showWhen==='before_during_after_attempt';return contextTools('card',{
...
function collectionItems(){
 const cardItems=CARD_GAMES.map(({id,title,description,color,pilot})=>({id,type:'cards',title,description,symbol:'◌',color,meta:cardsFor(id,true).length+(id==='tongue'?' tongbrekers · A0–C2':id==='idioms'?' kaarten · 240 Taalmix + 22 bestaande':' kaarten · 7 taalroutes')}));
