...
function startBoard(board){
 ...
 <label class="board-exercise-choice">Oefening<select class="mode-select" id="questionMode"><option value="direct">Snelvragen</option><option value="conversation">Met gesprekspartner</option><option value="mixed">Mix van beide</option></select></label>
 <label>Spelvorm<select class="mode-select" id="modeSelect"><option value="individual">Individueel</option><option value="class">Klassikaal</option><option value="groups">Groepen</option></select></label>
 <button class="smallbtn" id="nextBoardTask">Andere opdracht</button>
 <fieldset class="footer-choices"><legend>Bordweergave</legend>${[['fixed','Groot houden'],['adaptive','Alles zichtbaar']].map(([value,label])=>`<label class="footer-choice"><input type="radio" name="boardFit" value="${value}" ${(settingsState().boardFit==='adaptive'?'adaptive':'fixed')===value?'checked':''}><span>${label}</span></label>`).join('')}<small>Alles zichtbaar past het bord aan de vrije ruimte aan.</small></fieldset>
 <fieldset class="footer-choices"><legend>Gooiknop</legend>${[['blue','Blauw'],['red','Rood']].map(([value,label])=>`<label class="footer-choice"><input type="radio" name="boardFooter" value="${value}" ${(settingsState().boardFooter==='red'?'red':'blue')===value?'checked':''}><span class="footer-swatch ${value}" aria-hidden="true"></span><span>${label}</span></label>`).join('')}</fieldset>
