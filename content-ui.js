(function(root){
 ...
 function subtopics(){
  const functions=ContentRuntime.functionsFor({topic:state.topic,level:state.level});
  return [{id:'all',label:'Alles'},...functions.map(id=>({id,label:catalog.functionLabel(id)}))];
 }
 function filters(){
  return {topic:state.topic,level:state.level,language_functions:state.subtopic==='all'?[]:[state.subtopic],exercise_types:focus().exerciseTypes,productive_or_receptive:state.production,difficulty:state.difficulty};
 }
 ...
 function setState(patch,{render=true}={}){
  ...
  else if(state.level!==previousLevel){state.subtopic='all';state.profile=''}
  if(state.profile&&Object.keys(patch).some(k=>['level','subtopic','focus','production','difficulty'].includes(k)))state.profile='';
  if(!subtopics().some(x=>x.id===state.subtopic))state.subtopic='all';
 ...
 function selectField(name,label,items,value){return `<label class="practice-select"><span>${esc(label)}</span><select name="${esc(name)}">${items.map(item=>`<option value="${esc(item.id)}" ${item.id===value?'selected':''}>${esc(item.label)}</option>`).join('')}</select></label>`}
 function renderEngines(a,compatible,capacity){
  return `<fieldset class="practice-field practice-engines"><legend>Hoe wil je oefenen?</legend><div class="practice-engine-grid">${catalog.engines.map(item=>{const own=a.engines[item.id]?.count||0,enabled=compatible.includes(item.id)&&capacity>=state.duration;return `<label class="practice-engine ${state.engine===item.id?'selected':''} ${enabled?'':'disabled'}"><input type="radio" name="engine" value="${item.id}" ${state.engine===item.id?'checked':''} ${enabled?'':'disabled'}><strong>${esc(item.label)}</strong><span>${esc(item.description)}</span><small>${own} geschikte opdrachten${enabled?'':' · niet beschikbaar voor deze combinatie'}</small></label>`}).join('')}</div></fieldset>`;
 }
 ...
 function renderPage(){
  ...
  mount.innerHTML=`<div class="practice-layout"><form class="practice-config" id="practiceForm">
   ...
   <section class="practice-panel"><h2>2. Focus</h2>${choiceGroup('subtopic','Subonderwerp',subtopics(),state.subtopic)}${choiceGroup('focus','Oefenfocus',catalog.focuses,state.focus)}${choiceGroup('production','Productief of receptief',catalog.productionModes,state.production)}${choiceGroup('difficulty','Moeilijkheid',catalog.difficulties,state.difficulty)}</section>
 ...
 function sessionOptions(seed){
  ...
  return {seed:seed??seedOverride??Math.floor(Date.now()%4294967295),targetDurationSeconds:state.duration,engines:compatible,filters:filters(),organizationMode:state.organization,selectedGameEngine:state.engine,selectedGameVariant:state.variant,startedAt:new Date().toISOString()};
 }
 function start(seed){
  try{
   const options=sessionOptions(seed),controller=root.CONTENT_GRAMMAR||root.CONTENT_VERT001,session=controller.start(options);
   APP.turn.mode=state.organization;settingsPatch({pawnMode:state.organization});if(typeof selectLevel==='function')selectLevel(state.level);else{APP.level=state.level;save()}
   if(state.engine==='BOARD')controller.board(state.variant||'rotterdam');else if(state.engine==='WHEEL')controller.wheel();else if(state.engine==='CARDS')controller.cards();
   return session;
 ...
 document.addEventListener('click',e=>{const direct=e.target.closest('[data-practice-open]');if(direct&&!direct.disabled){e.preventDefault();open();return}const entry=e.target.closest('[data-practice-engine]');if(entry&&!entry.disabled){e.preventDefault();open({engine:entry.dataset.practiceEngine,variant:entry.dataset.practiceVariant})}},true);
 document.addEventListener('click',e=>{const launcher=e.target.closest('[data-board],[data-cardgame],[data-dicegame],[data-wordgame],[data-activity]');if(launcher&&!launcher.closest('#screen-practice')&&ContentRuntime.activeSession?.())(root.CONTENT_GRAMMAR||root.CONTENT_VERT001).stop()},true);
 try{if(APP.contentSessionConfig)(root.CONTENT_GRAMMAR||root.CONTENT_VERT001).restore()}catch{delete APP.contentSessionConfig;delete APP.contentVert001Used;save()}
