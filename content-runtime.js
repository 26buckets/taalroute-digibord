(function(root,factory){
 ...
 else root.ContentRuntime=factory(root.DIGIBORD_CONTENT_VERT001,{familyId:'grammar'});
 ...
 const banks=new Map(),byId=new Map(),itemBanks=new Map(),domains=new Map();let active=null;
 ...
 function registerBank(bank,metadata={}){
  ...
  for(const item of bank.items){byId.set(item.content_item_id,item);itemBanks.set(item.content_item_id,entry);for(const key of ['topic','cefr_level','technical_tags','language_function','exercise_type','interaction_type','support_level','oral_or_written']){if(!domains.has(key))domains.set(key,new Set());for(const value of list(item[key]))domains.get(key).add(value)}}
  return entry;
 }
 registerBank(source,{familyId:options.familyId});
 ...
 function eligible(item){return (itemBanks.get(item.content_item_id).reviewGate||PROFILE.reviewGate).includes(item.review_status)&&PROFILE.publicationGate.includes(item.publication_status)&&['owned_original','licensed'].includes(item.rights_status)&&!['SOFT_DEPRECATED','HARD_REVOKED'].includes(item.revocation_status)}
 ...
 function setCompatibility(pool,engine){
  if(!pool.length||pool.some(item=>!compatibility(item,engine).compatible))return false;
 ...
 function selectItems({seed=20260922,targetDurationSeconds=600,engines=null,filters={},selectionSpec=null}={}){
  ...
  const random=rng(seed),buckets=[];
  for(const {scope,pool:scopePool} of scopes){for(const fn of unique(scopePool.map(i=>i.language_function)))buckets.push({scope,fn,items:shuffle(scopePool.filter(i=>i.language_function===fn),random),count:0})}
  const selected=[],used=new Set(),counts=new Map(scopes.map(s=>[s.scope.scope_id,{count:0,duration:0}]));let total=0;
  const minimumMet=()=>scopes.every(({scope})=>{const s=counts.get(scope.scope_id);return s.count>=Math.max(1,scope.minimum_items||0)&&s.duration>=(scope.minimum_duration_seconds||0)})&&buckets.every(b=>b.count>0);
 ...
 function contentRef(item){const bank=itemBanks.get(item.content_item_id).bank;return {content_item_id:item.content_item_id,content_item_version:item.version,content_bank_id:item.content_bank_id,source_version:bank.source_version,immutable_version_ref:bank.bank_id+'@'+itemBanks.get(item.content_item_id).immutableKey+'/'+item.content_item_id+'@'+item.version,content_hash:fingerprint(contentData(item))}}
 function validateContentRefs(refs,{historical=false}={}){
  ...
  for(const ref of refs){const item=byId.get(ref.content_item_id);if(!item)fail('BLOCKED_VERSION_UNAVAILABLE','Deze oude inhoudsversie is niet beschikbaar.');
   if(item.revocation_status==='HARD_REVOKED'||!['owned_original','licensed'].includes(item.rights_status))fail('BLOCKED_RIGHTS','Een onderdeel uit deze sessie is ingetrokken.');
   if(!historical&&!eligible(item))fail('BLOCKED_CONTENT_STATUS','Een onderdeel is niet beschikbaar voor een nieuwe sessie.');
   if(stable(contentRef(item))!==stable(ref))fail('BLOCKED_VERSION_UNAVAILABLE','Deze oude inhoudsversie is niet beschikbaar.');
  }return true;
 }
 const newSessionId=()=> 'CONTENT-'+(globalThis.crypto?.randomUUID?.()||Date.now().toString(36)+'-'+Math.random().toString(36).slice(2));
 function createSession({seed=20260922,targetDurationSeconds=600,engines=null,filters={},selectionSpec=null,organizationMode='class',selectedGameEngine=null,selectedGameVariant=null,startedAt=null,selectionTopic=null}={}){
  ...
  const selected=selectItems({seed,targetDurationSeconds:Number(targetDurationSeconds),engines:resolved,filters:f,selectionSpec:spec}),bankIds=unique(selected.map(i=>i.content_bank_id)),topics=unique(selected.map(i=>i.topic)),levels=unique(selected.map(i=>i.cefr_level)),families=unique(selected.map(i=>itemBanks.get(i.content_item_id).familyId));
 ...
 function restoreSession(config){
  ...
  if(!refs){const bank=banks.get(config.content_bank_id)?.bank;if(!bank||config.content_source?.version!==bank.source_version||config.content_source?.source_sha256!==bank.source_sha256)fail('BLOCKED_VERSION_UNAVAILABLE','De oorspronkelijke inhoudsversie ontbreekt.');refs=config.selected_item_ids.map(id=>{const i=byId.get(id);if(!i)fail('BLOCKED_VERSION_UNAVAILABLE','Een oorspronkelijke opdracht ontbreekt.');return contentRef(i)})}
  ...
  for(const engine of config.game_engines)if(!engineRegistry.supportsOrganization(engine,config.organization_mode)||!setCompatibility(refs.map(r=>byId.get(r.content_item_id)),engine))fail('BLOCKED_COMPATIBILITY','De opgeslagen spelvorm kan deze sessie niet uitvoeren.');
 ...
 function enginePool(engine,session){
  ...
  const pool=session.selected_item_ids.map(id=>{const item=byId.get(id);if(!item)fail('BLOCKED_VERSION_UNAVAILABLE','Ontbrekend content_item_id: '+id);return item});
  if(!setCompatibility(pool,engine))fail('BLOCKED_COMPATIBILITY','Incompatibele inhoud voor '+engine);return pool;
 }
 ...
 function project(engine,item){const check=compatibility(item,engine);if(!check.compatible)fail('BLOCKED_COMPATIBILITY','Niet compatibel: '+item.content_item_id+' voor '+engine);const part=String(item.prompt).slice(String(item.prompt).lastIndexOf(':')+1);return {engine,contentItemId:item.content_item_id,sourceItem:item,prompt:item.prompt,options:item.options,interactionType:item.interaction_type,renderer:check.renderer,compatibility:check,adapter:check.adapter,orderTokens:item.order_tokens|| (check.adapter==='text_order'?part.split('/').map(x=>x.trim()).filter(Boolean):[]),orderExpectedTokens:item.expected_tokens||(check.adapter==='text_order'?String(item.correct_answer).trim().replace(/[.!?]+$/,'').split(/\s+/):[]),pair:['MATCH','MEMORY'].includes(engine)?pairContract.candidate(item).pair:null,answerPolicy:answerPolicy(item)}}
 ...
 return Object.freeze({PROFILE,get ENGINE_VERSIONS(){return Object.fromEntries(currentEngines().map(e=>[e,engineRegistry.get(e).version]))},get TOPICS(){return values('topic')},get LEVELS(){return values('cefr_level')},get FUNCTIONS(){return values('language_function')},source,registerBank,banks:()=>[...banks.values()],items,compatibility,setCompatibility,normalizeFilters,filterSource,eligibleItems,availability,fullCoverageEngines,selectItems,normalizeSelection,specFromFilters,selectionPool,compatibleSelectionEngines,createSession,restoreSession,enginePool,answerPolicy,project,nextItem,activateSession:o=>(active=createSession(o)),activeSession:()=>active,clearSession:()=>{active=null},itemById:id=>byId.get(id)||null,contentRef,validateContentRefs,newSessionId,fingerprint,stable,fail});
});
