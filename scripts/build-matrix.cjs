...
const direct=JSON.parse(fs.readFileSync(path.join(dir,'directe-vragen.json'),'utf8'));
assert.equal(direct.cards.length,240);assert.equal(new Set(direct.cards.map(c=>c.id)).size,240);
for(const c of direct.cards){assert.ok(b.routes.some(r=>r.id===c.routeId));assert.ok(b.shapes.some(s=>s.id===c.shape));for(const k of ['instruction','support','model','grammar','retry','criterion'])assert.ok(c[k]?.trim(),c.id+' missing '+k);}
fs.writeFileSync(path.join(dir,'directe-vragen-data.js'),'globalThis.DigiBoardDirectContent='+JSON.stringify(direct)+';\n');
