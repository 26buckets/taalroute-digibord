...
reg=app/'Kaarten/register.js';payload=reg.read_text().split('=',1)[1];old,cut=json.JSONDecoder().raw_decode(payload);register_tail=payload[cut:];ids={c['id'] for c in C};old=[x for x in old if x['id']not in ids];old.extend(dict(id=c['id'],label=c['label'],count=c['count'],category='nederland',image='Kaarten/assets/'+c['id']+'/'+c['image']+'.png',features=list(dict.fromkeys(['Pont' if p['kind']=='ferry' else 'Boot' if p.get('boat') else 'Tunnel' for p in c['passages']]+(['Brug']if any(m.get('bridge')for m in c['masks'])else[]))))for c in C)
reg.write_text('globalThis.DigiBoardMaps='+json.dumps(old,ensure_ascii=False,separators=(',',':'))+register_tail)
