...
globalThis.DigiBoard=(()=>{
 ...
 function connect(h){host=h;const picker=document.createElement('button');picker.id='db-open-maps';picker.type='button';picker.textContent='Kaarten';picker.onclick=()=>{if(!h.busy())h.openMapPicker();};document.getElementById('pp-main-tools').prepend(picker);const select=document.getElementById('db-map');select.replaceChildren(...DigiBoardMaps.map(m=>{const o=document.createElement('option');o.value=m.id;o.textContent=m.label+' · '+m.count+' vakken';return o;}));select.value=mapId;select.onchange=()=>{if(h.busy()){select.value=mapId;return;}h.read().settings.diceStyle='numbers';h.chooseMap(select.value);};
  ...
  if(mapId==='Rotterdam-havenroute'||globalThis.DigiBoardMap?.special){const b=document.createElement('button');b.id='map-rules-button';b.type='button';b.textContent='Speciale plekken';b.onclick=()=>h.showMapRules();document.getElementById('pp-main-tools').prepend(b);}
 ...
 function sync(d){const form=d.settings.diceStyle||'numbers';const picker=document.getElementById('db-open-maps');if(picker)picker.disabled=!!host?.busy();const root=document.getElementById('praatpad-board');root.dataset.dbForm=d.settings.diceStyle||'numbers';for(const id of ['db-map','db-board-map']){const e=document.getElementById(id);if(e){e.value=mapId;e.disabled=!!host?.busy();}}const help=document.getElementById('world-route-help');if(help)help.hidden=form!=='numbers';const rules=document.getElementById('map-rules-button');if(rules){rules.hidden=form!=='numbers';rules.disabled=!!host?.busy();}const el=document.querySelector('.db-map-picker');if(el)el.hidden=form!=='numbers';}
