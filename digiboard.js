...
globalThis.DigiBoard=(()=>{
 ...
 function sync(d){const form=d.settings.diceStyle||'numbers';const picker=document.getElementById('db-open-maps');if(picker)picker.disabled=!!host?.busy();const root=document.getElementById('praatpad-board');root.dataset.dbForm=form==='sentence'?'numbers':form;root.dataset.dbExercise=form;document.getElementById('db-map-tools').hidden=!['numbers','sentence'].includes(form);for(const id of ['db-map','db-board-map']){const e=document.getElementById(id);if(e){e.value=mapId;e.disabled=!!host?.busy();}}const help=document.getElementById('world-route-help');if(help)help.hidden=!['numbers','sentence'].includes(form);const rules=document.getElementById('map-rules-button');if(rules){rules.hidden=!['numbers','sentence'].includes(form);rules.disabled=!!host?.busy();}const el=document.querySelector('.db-map-picker');if(el)el.hidden=!['numbers','sentence'].includes(form);}
