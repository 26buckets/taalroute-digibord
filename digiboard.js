...
globalThis.DigiBoard=(()=>{
 ...
 const wanted=new URLSearchParams(location.search).get('kaart')||read(ACTIVE)||'buurt';
 const mapId=DigiBoardMaps.some(m=>m.id===wanted)?wanted:'buurt';let host;
 ...
 function prepare(){
  ...
  const map=document.getElementById('pp-map');map.setAttribute('aria-label','Speelroute · '+m.label);map.insertAdjacentHTML('beforeend',m.extras);
  const style=document.createElement('link');style.rel='stylesheet';style.href='Kaarten/'+mapId+'.css';document.head.append(style);
  map.querySelectorAll('.rotterdam-bridge-layer image').forEach(i=>i.setAttribute('href',scene.src));
 ...
 function connect(h){host=h;const select=document.getElementById('db-map');select.replaceChildren(...DigiBoardMaps.map(m=>{const o=document.createElement('option');o.value=m.id;o.textContent=m.label+' · '+m.count+' vakken';return o;}));select.value=mapId;select.onchange=()=>{if(h.busy()){select.value=mapId;return;}h.read().settings.diceStyle='numbers';h.chooseMap(select.value);};
  if(globalThis.DigiBoardMap){const b=document.createElement('button');b.id='world-route-help';b.type='button';b.textContent='Toon vaknummers';b.setAttribute('aria-pressed','false');b.onclick=()=>{const r=document.getElementById('praatpad-board'),on=r.dataset.routeHelp!=='true';r.dataset.routeHelp=String(on);b.setAttribute('aria-pressed',String(on));b.textContent=on?'Verberg vaknummers':'Toon vaknummers';};document.getElementById('pp-main-tools').prepend(b);}
  ...
  if(mapId==='Rotterdam-havenroute'){const b=document.createElement('button');b.id='map-rules-button';b.type='button';b.textContent='Speciale plekken';b.onclick=()=>h.showMapRules();document.getElementById('pp-main-tools').prepend(b);}
 ...
 function sync(d){const root=document.getElementById('praatpad-board');root.dataset.dbForm=d.settings.diceStyle||'numbers';for(const id of ['db-map','db-board-map']){const e=document.getElementById(id);if(e){e.value=mapId;e.disabled=!!host?.busy();}}const help=document.getElementById('world-route-help');if(help)help.hidden=d.settings.diceStyle!=='numbers';const rules=document.getElementById('map-rules-button');if(rules){rules.hidden=d.settings.diceStyle!=='numbers';rules.disabled=!!host?.busy();}const el=document.querySelector('.db-map-picker');if(el)el.hidden=d.settings.diceStyle!=='numbers';}
