...
const {bank,originals,review,output}=require('../scripts/import-vergadertaal.cjs'),{createContentRuntime}=require('../content-runtime.js'),guidance=require('../content-guidance.js');
assert.equal(fs.readFileSync(require.resolve('../data/vergadertaal.js'),'utf8'),output);
...
for(const [n,i] of bank.items.entries()){
 ...
 assert.equal(i.content_item_id,'C1_VB_'+String(n+1).padStart(3,'0'));for(const k of ['title','context','prompt'])assert.equal(i[k],task[k]);assert.deepEqual(i.options,task.options.map((s,n)=>'ABC'[n]+'. '+s));assert.equal(i.model_answer,i.options['ABC'.indexOf(source.letter)]);
...
assert.deepEqual(levels,{B2:44,C1:16});assert.deepEqual(counts,{B:20,A:20,C:20});assert.equal(Object.values(review.items).filter(r=>Object.keys(r.patch).length).length,20);
assert.match(runtime.itemById('C1_VB_001').context,/Wat is het doel van dit agendapunt/);assert.ok(!runtime.itemById('C1_VB_020').prompt.includes('titel'));assert.equal(runtime.itemById('C1_VB_043').title,'Zijn er genoeg leden aanwezig?');
assert.equal(bank.items.filter(i=>i.correct_answer===null).length,9);
...
const previousBanks=[old.bank,workplace.bank,require('../data/content-vert001-er-b1.js'),require('../data/gram-pb003.js'),require('../data/gram-pb002.js'),require('../data/wz-pb003.js'),require('../data/wz-pb004.js'),require('../data/wz-pb005.js'),adapt(require('../Lessen/woorden-zinnen.json')),adapt(require('../data/wz-pb002.js')),require('../data/connections-pilot.js'),adapt.riddles(activityContext.window.DIGIBORD_ACTIVITIES)];
assert.equal(previousBanks.reduce((n,b)=>n+b.items.length,0),5271);
...
console.log('PASS Vergadertaal: 60 source IDs, frozen hash, 20 corrections, B2/C1 44/16, old banks unchanged, overlap against 5271 + 50, lesson guidance, both routes and game restrictions.');
