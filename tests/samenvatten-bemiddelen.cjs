...
const {bank,originals,review,output}=require('../scripts/import-samenvatten-bemiddelen.cjs'),{createContentRuntime}=require('../content-runtime.js'),guidance=require('../content-guidance.js');
assert.equal(fs.readFileSync(require.resolve('../data/samenvatten-bemiddelen.js'),'utf8'),output);
...
for(const [n,i] of bank.items.entries()){
 ...
 assert.equal(i.content_item_id,'C1_SB_'+String(n+1).padStart(3,'0'));for(const k of ['title','context','prompt'])assert.equal(i[k],task[k]);assert.deepEqual(i.options,task.options.map((s,n)=>'ABC'[n]+'. '+s));assert.equal(i.model_answer,i.options['ABC'.indexOf(task.letter)]);
...
assert.deepEqual(levels,{B1:14,B2:36});assert.deepEqual(counts,{A:17,B:17,C:16});assert.equal(Object.values(review.items).filter(r=>Object.keys(r.patch).length).length,50);
assert.equal(bank.items.filter(i=>i.correct_answer===null).length,16);
...
const card=n=>runtime.itemById('C1_SB_'+String(n).padStart(3,'0'));
for(const i of bank.items)assert.ok(!/de (eerste|tweede|derde) (versie|optie|stap)/i.test(i.explanation),'No stale answer position');
assert.match(card(2).model_answer,/positief over de proef/);
assert.match(card(6).model_answer,/tegen invoering op 1 november/);
assert.ok(!/vaste/.test(card(18).model_answer));assert.ok(!/onvoorwaardelijk/.test(card(22).model_answer));
assert.match(card(23).model_answer,/huidige totaalbedrag/);
assert.match(card(35).context,/productie- en voorraadrisico/);assert.match(card(35).context,/financieel risico/);
assert.match(card(38).model_answer,/Ben je bang/);assert.equal(card(38).correct_answer,null);
assert.ok(!/leggen die nu/.test(card(41).model_answer));assert.match(card(42).model_answer,/Voorstellen/);
assert.ok(!/instemming volgt/.test(card(44).model_answer));assert.match(card(44).explanation,/afwijzing/);
assert.equal(card(50).correct_answer,null);assert.match(card(50).explanation,/geen vaste regel/);
...
console.log('PASS Samenvatten en bemiddelen: 50 source IDs, frozen hash, source-faithful summaries, B1/B2 14/36, no automatic C1 or audio, old banks unchanged, overlap against 5631 + 50, guidance, routes and game restrictions.');
