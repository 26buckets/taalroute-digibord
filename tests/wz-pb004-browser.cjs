...
(async()=>{
 ...
 await page.locator('[name=family]').selectOption('words');await page.locator('[name=topic]').selectOption('WZ_022');await page.locator('[name=level]').selectOption('A2');await page.locator('.practice-engine:has(input[value=CARDS])').click();
 assert.equal(await page.evaluate(()=>ContentRuntime.filterSource({bank_ids:['CB-WZ-004']}).length),598);
 assert.equal(await page.locator('.practice-engine input[value=DICE]').count(),0);
 assert.deepEqual(await page.locator('[name=level] option').evaluateAll(es=>es.map(e=>e.value)),['A2']);
 ...
 await page.setViewportSize({width:1440,height:1000});const dir=path.join(root,'tests/artifacts/wz-pb004');fs.mkdirSync(dir,{recursive:true});await page.screenshot({path:path.join(dir,'preparation.png')});
 await page.locator('#practiceSave').click();await page.locator('#lessonName').fill('Scheidbare werkwoorden');await page.locator('#lessonSaveForm .primary').click();await page.waitForFunction(()=>!document.querySelector('#gameDialog').open);
 ...
 await page.evaluate(()=>{ContentUI.clearEditing();const filters={bank_ids:['CB-WZ-004']},pool=ContentRuntime.filterSource(filters);ContentUI.launch(ContentRuntime.createSession({filters,targetDurationSeconds:pool.reduce((n,i)=>n+i.estimated_duration_seconds,0),selectedGameEngine:'CARDS'}));settingsPatch({reducedMotion:true})});
 const seen=new Set();for(let n=0;n<598;n++){
 ...
 assert.equal(seen.size,598);
 for(const engine of ['BOARD','WHEEL','QUIZ','SEQUENCE']){
  await page.evaluate(engine=>{const filters={bank_ids:['CB-WZ-004'],levels:['A2'],exercise_types:engine==='SEQUENCE'?['zinnen_leggen']:[]};APP.fixedRoll=1;ContentUI.launch(ContentRuntime.createSession({filters,targetDurationSeconds:300,selectedGameEngine:engine,organizationMode:'groups',seed:11}))},engine);
  await page.locator('[data-ghelp]').click();assert.equal(await page.locator('.guidance-status').textContent(),'A2');await page.keyboard.press('Escape');
 ...
 assert.deepEqual(errors,[]);console.log('PASS WZ PB004 browser: all 598 cards, five screen sizes, actual lesson, both routes, save/reload, four other engines, guidance and source IDs.');
