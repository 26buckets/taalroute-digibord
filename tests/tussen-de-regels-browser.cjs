...
(async()=>{
 ...
 await page.goto(`http://127.0.0.1:${server.address().port}/index.html`);await page.locator('[data-category=cards]').click();await page.locator('[data-ctype="c1-between-lines"]').click();
 assert.equal(await page.locator('[name=topic]').inputValue(),'tussen-de-regels');assert.equal(await page.evaluate(()=>ContentRuntime.items().length),8414);
 const combined=await page.evaluate(()=>ContentRuntime.filterSource({family_ids:['conversation']}));assert.equal(combined.length,690);assert.equal(new Set(combined.map(i=>i.content_item_id)).size,690);assert.equal(combined.filter(i=>i.cefr_level==='B1').length,153);assert.equal(combined.filter(i=>i.cefr_level==='B2').length,537);
 await page.locator('[name=family]').selectOption('conversation');
 assert.deepEqual(await page.locator('[name=level] option').allTextContents(),['B1','B2']);
 await page.locator('[name=topic]').selectOption('tussen-de-regels');assert.deepEqual(await page.locator('[name=level] option:not([disabled])').allTextContents(),['B1','B2']);await page.locator('[name=level]').selectOption('B2');
 ...
 await page.setViewportSize({width:1440,height:1000});const dir=path.join(root,'tests/artifacts/tussen-de-regels');fs.mkdirSync(dir,{recursive:true});await page.screenshot({path:path.join(dir,'preparation.png')});
 await page.locator('#practiceSave').click();await page.locator('#lessonName').fill('Nederlands tussen de regels');await page.locator('#lessonSaveForm .primary').click();await page.waitForFunction(()=>!document.querySelector('#gameDialog').open);
 ...
 await page.evaluate(()=>{ContentUI.clearEditing();const filters={bank_ids:['CB-BETWEEN-LINES-012']};ContentUI.launch(ContentRuntime.createSession({filters,targetDurationSeconds:ContentRuntime.filterSource(filters).reduce((n,i)=>n+i.estimated_duration_seconds,0),selectedGameEngine:'CARDS'}))});
 ...
 for(const engine of ['BOARD','WHEEL']){
  await page.evaluate(engine=>{APP.fixedRoll=1;settingsPatch({reducedMotion:true});ContentUI.launch(ContentRuntime.createSession({filters:{family_ids:['conversation'],topics:['tussen-de-regels'],levels:['B2']},targetDurationSeconds:600,selectedGameEngine:engine,seed:11}))},engine);
  ...
  if(engine==='BOARD'){await page.locator('#primaryGame').click();await page.waitForFunction(()=>!boardBusy&&document.querySelector('#taskDrawer.open'));assert.match(await page.locator('#taskDrawer').getAttribute('data-task-id'),/^C1_(AL|INT|REG|VER|IMP)_/);}
  if(engine==='WHEEL'){
   assert.equal(await page.locator('.na-wheel-legend [data-content-item-id]').count(),6);const spun=new Set();
   for(let n=0;n<12;n++){await page.locator('#primaryGame').click();await page.waitForFunction(()=>!document.querySelector('#primaryGame').disabled);const id=await page.locator('.na-wheel-result [data-content-item-id]').getAttribute('data-content-item-id');spun.add(id);assert.match(id,/^C1_(AL|INT|REG|VER|IMP)_/);}
 ...
 assert.deepEqual(errors,[]);console.log('PASS Nederlands tussen de regels browser: 50 cards, actual lesson and counts, both routes, guidance, suitable games, saving, progress/reload, board/wheel and five screen sizes.');
