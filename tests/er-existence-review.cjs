...
const original=require('../data/content-vert001-er-b1.js'),api=require('../data/grammar-review.js'),review={...api,revise:api.existence,version:api.existenceVersion},{createContentRuntime}=require('../content-runtime.js');
const previous=review.passive(original),bank=review.revise(original),hash=x=>crypto.createHash('sha256').update(JSON.stringify(x)).digest('hex');
assert.equal(hash(previous.items),'9cbc38dd4748b00f97c9fa38aac56ee6f72d7229f7d0d653e9b1594ffb711b68');assert.equal(hash(bank.items),bank.source_sha256);
const current=createContentRuntime(bank,{familyId:'grammar',previousVersions:[original,review.previous(original),review.references(original),previous]});
...
for(const item of bank.items){
 ...
 assert.equal(item.source_ref.original_cefr_level,'B2');assert.equal(item.level_review.level,'B1');assert.equal(item.language_function,'abstract_existenteel');
...
for(const [id,pattern] of [['038',/ons werk/],['039',/kunnen proberen/],['048',/startdatum.*contactpersoon/],['049',/bewijs dat.*veroorzaakt/],['050',/uitstapje/],['051',/onderhoudsboekje ontbreekt/],['060',/geld en medewerkers/]])assert.match(current.itemById('ER_B2_'+id).model_answer,pattern);
assert.match(current.itemById('ER_B2_053').context,/van tien tot elf/);assert.match(current.itemById('ER_B2_060').explanation,/zeker doorgaat/);
assert.equal(current.filterSource({topics:['ER'],levels:['B1']}).length,262);assert.equal(current.filterSource({levels:['B2']}).length,398);
assert.equal(require('../match-pair-contract.js').audit(set).pairs.length,6);
for(const source of [original,review.previous(original),review.references(original),previous]){
 const old=createContentRuntime(source,{familyId:'grammar'}),filters={topics:['ER'],levels:['B2'],language_functions:['abstract_existenteel']};
 ...
 const saved=old.specFromFilters({...filters,exercise_types:['invullen']}),updated=current.currentSelection(saved);assert.deepEqual(updated.scope_clauses[0].cefr_levels,['B1']);assert.deepEqual(current.selectionPool(updated).map(i=>i.content_item_id).sort(),['ER_B2_032','ER_B2_044','ER_B2_056']);
}
...
console.log('PASS 30 existence Er tasks: concrete context, B1 advice, no answer leaks, six closed pairs, grammar/meaning repairs, other 1410 unchanged and four historical versions restored across five engines.');
