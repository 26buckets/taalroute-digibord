...
assert.deepEqual(engines.contentEngines().map(x=>x.id),['BOARD','WHEEL','CARDS','DICE','QUIZ','MEMORY','MATCH','SORT','SEQUENCE','RIDDLE']);
...
assert.deepEqual(engines.all().filter(x=>!x.contentSessionEnabled).map(x=>x.id),['TAALWORP','STORY_DICE','WORDS'],'specialized engines remain visible without false universal compatibility');
...
for(const engine of engines.contentEngines().filter(x=>['BOARD','WHEEL','CARDS','DICE','QUIZ'].includes(x.id)))assert.ok(source.items.every(item=>renderers.compatibility(item,engine.id).compatible),engine.id+' covers current PB001 interactions');
...
const session=runtime.createSession({seed:1001,targetDurationSeconds:300,engines:['BOARD','WHEEL','CARDS','DICE','QUIZ','SEQUENCE'],filters:{topics:['ER'],levels:['B1'],exercise_types:['zinnen_leggen']},selectionTopic:'ER',organizationMode:'groups'});
const ids=session.selected_item_ids;
for(const engine of engines.contentEngines().filter(e=>session.game_engines.includes(e.id)))assert.deepEqual(runtime.enginePool(engine.id,session).map(x=>x.content_item_id),ids,engine.id+' receives one shared SessionConfig');
