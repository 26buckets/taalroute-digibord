...
const original=require('../data/content-vert001-er-b1.js'),api=require('../data/grammar-review.js'),review={...api,revise:api.passive,version:api.passiveVersion},{createContentRuntime}=require('../content-runtime.js');
const previous=review.references(original),bank=review.revise(original),hash=x=>crypto.createHash('sha256').update(JSON.stringify(x)).digest('hex');
assert.equal(hash(previous.items),'c2ea508b92cc934514d9567947e54d1068cadcaf0ae4ea45de86adecc3a18b62');assert.equal(hash(bank.items),bank.source_sha256);
const current=createContentRuntime(bank,{familyId:'grammar',previousVersions:[original,review.previous(original),previous]});
...
for(const item of bank.items){
 ...
 assert.equal(item.source_ref.original_cefr_level,'B2');assert.equal(item.level_review.level,'B1');assert.equal(item.language_function,'onpersoonlijk_passief');
...
for(const [id,pattern] of [['007',/vandaag/],['008',/door medewerkers/],['009',/morgen.*kunnen/],['019',/door ziekte/],['020',/door de medewerkers.*gevraagd/],['021',/volgende maand/],['023',/twee weken langer/]])assert.match(current.itemById('ER_B2_'+id).model_answer,pattern);
assert.equal(current.filterSource({topics:['ER'],levels:['B1']}).length,232);assert.equal(current.filterSource({levels:['B2']}).length,428);
assert.equal(require('../match-pair-contract.js').audit(set).pairs.length,6);
for(const source of [original,review.previous(original),previous]){
 const old=createContentRuntime(source,{familyId:'grammar'}),filters={topics:['ER'],levels:['B2'],language_functions:['onpersoonlijk_passief']};
 ...
 const saved=old.specFromFilters({...filters,exercise_types:['invullen']}),updated=current.currentSelection(saved);assert.deepEqual(updated.scope_clauses[0].cefr_levels,['B1']);assert.deepEqual(current.selectionPool(updated).map(i=>i.content_item_id).sort(),['ER_B2_002','ER_B2_014','ER_B2_026']);
}
...
console.log('PASS 30 passive Er tasks: concrete context, B1 advice, no answer leaks, six closed pairs, grammar/meaning repairs, other 1410 unchanged and three historical versions restored across five engines.');
