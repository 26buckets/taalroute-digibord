...
const source=require('../data/snelvragen-content.js'),direct=require('../data/snelvragen-review.js').revise(source),firstReview=require('../data/snelvragen-vertel-review.js'),first=firstReview.revise(direct),secondReview=require('../data/snelvragen-vertel-vervolg.js'),previous=secondReview.revise(first),review=require('../data/snelvragen-vertel-situaties.js'),{createContentRuntime}=require('../content-runtime.js'),guidance=require('../content-guidance.js');
...
assert.deepEqual(ids,source.items.filter(i=>i.topic==='quick-tell'&&!firstReview.reviews[i.content_item_id]&&!secondReview.reviews[i.content_item_id]).slice(0,120).map(i=>i.content_item_id));assert.equal(ids.length,120);assert.equal(JSON.stringify(previous),snapshot);
assert.deepEqual(bank.items.filter(i=>!idSet.has(i.content_item_id)),previous.items.filter(i=>!idSet.has(i.content_item_id)),'2501 other tasks unchanged, including previous 480 reviewed questions');assert.equal(bank.items.length,2621);
const runtime=createContentRuntime(require('../data/tussen-de-regels.js'));runtime.registerBank(bank,{...meta,previousVersions:[source,direct,first,previous]});
for(const b of [source,direct,first,previous])guidance.registerHistorical(b);guidance.register(bank.guidance);guidance.complete(bank.items);
...
assert.equal(models,1);assert.equal(changedModels,0);assert.equal(changedLevels,76);
assert.deepEqual(ids.reduce((a,id)=>(a[runtime.itemById(id).cefr_level]=(a[runtime.itemById(id).cefr_level]||0)+1,a),{}),{A2:44,A1:74,B1:2});
for(const oldBank of [source,direct,first,previous]){
...
assert.match(get('2-037').context,/9.25.*9.15.*9.45/);assert.match(get('2-038').context,/station.*bakker.*rechtsaf.*park/);assert.match(get('2-028').context,/twee euro.*twee euro vijftig/);assert.match(get('2-081').context,/tien uur.*half elf/);assert.match(get('2-081').explanation,/niet welke tijd juist/);assert.match(get('2-095').context,/Betaling mislukt/);assert.match(get('2-120').explanation,/niet automatisch lek/);assert.match(get('3-003').prompt,/weet je nog niet/);assert.match(get('3-004').explanation,/mogelijk, niet bewezen/);assert.equal(get('2-073').model_answer,source.items.find(i=>i.content_item_id==='sq-r2-circle-073').model_answer);
...
console.log('PASS third 120 Vertel: 74 A1/44 A2/2 B1; 76 level repairs, all source models unchanged; individual goals/guidance, other 2501 unchanged, 1440 exact historical restores plus pre-ref sessions, three-engine parity and theme spread.');
