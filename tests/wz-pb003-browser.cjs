...
(async()=>{
 ...
 await page.locator('[name=family]').selectOption('words');await page.locator('[name=topic]').selectOption('WZ_011');await page.locator('[name=level]').selectOption('A1');await page.locator('.practice-engine:has(input[value=CARDS])').click();
 assert.equal(await page.evaluate(()=>ContentRuntime.filterSource({bank_ids:['CB-WZ-003']}).length),425);
 assert.equal(await page.locator('.practice-engine input[value=DICE]').count(),0);
 assert.deepEqual(await page.locator('[name=level] option').evaluateAll(es=>es.map(e=>e.value)),['A1']);
 assert.equal(await page.evaluate(()=>ContentRuntime.filterSource({topics:['WZ_011'],levels:['A2']}).length),0);
 assert.ok(!(await page.locator('#practiceForm').textContent()).includes('Oefen stap voor stap naar A1.'));
 await page.locator('[name=topic]').selectOption('WZ_014');await page.locator('[name=level]').selectOption('A2');assert.ok(!(await page.locator('#practiceForm').textContent()).includes('Oefen stap voor stap naar A1.'));await page.locator('[name=topic]').selectOption('WZ_011');await page.locator('[name=level]').selectOption('A1');
 ...
 await page.setViewportSize({width:1440,height:1000});const dir=path.join(root,'tests/artifacts/wz-pb003');fs.mkdirSync(dir,{recursive:true});await page.screenshot({path:path.join(dir,'preparation.png')});
 await page.locator('#practiceSave').click();await page.locator('#lessonName').fill('Zinnen uitbreiden');await page.locator('#lessonSaveForm .primary').click();await page.waitForFunction(()=>!document.querySelector('#gameDialog').open);
 ...
 await page.evaluate(()=>{ContentUI.clearEditing();const filters={bank_ids:['CB-WZ-003']},pool=ContentRuntime.filterSource(filters);ContentUI.launch(ContentRuntime.createSession({filters,targetDurationSeconds:pool.reduce((n,i)=>n+i.estimated_duration_seconds,0),selectedGameEngine:'CARDS'}));settingsPatch({reducedMotion:true})});
 const seen=new Set();for(let n=0;n<425;n++){
 ...
 for(const engine of ['BOARD','WHEEL','QUIZ','SEQUENCE']){
  await page.evaluate(engine=>{const filters={bank_ids:['CB-WZ-003'],levels:['A1'],exercise_types:engine==='SEQUENCE'?['zinnen_leggen']:[]};APP.fixedRoll=1;ContentUI.launch(ContentRuntime.createSession({filters,targetDurationSeconds:300,selectedGameEngine:engine,organizationMode:'groups',seed:11}))},engine);
  await page.locator('[data-ghelp]').click();assert.equal(await page.locator('.guidance-status').textContent(),'A1');await page.keyboard.press('Escape');
 ...
 assert.deepEqual(errors,[]);console.log('PASS WZ PB003 browser: all 425 cards, five screen sizes, actual lesson, both routes, save/reload, four other engines, guidance and source IDs.');
