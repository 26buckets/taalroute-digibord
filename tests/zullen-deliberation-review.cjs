...
const original=require('../data/content-vert001-er-b1.js'),api=require('../data/grammar-review.js'),review={...api,revise:api.deliberation,version:api.deliberationVersion},{createContentRuntime}=require('../content-runtime.js');
const previous=review.certainty(original),bank=review.revise(original),hash=x=>crypto.createHash('sha256').update(JSON.stringify(x)).digest('hex');
assert.equal(hash(previous.items),'3b093690264dcabe5763d94cd8966562ebae7c6748a800a66bd0062ab7096221');assert.equal(hash(bank.items),bank.source_sha256);
const current=createContentRuntime(bank,{familyId:'grammar',previousVersions:[original,review.previous(original),review.references(original),review.passive(original),review.existence(original),review.appearance(original),review.reporting(original),review.argument(original),review.probability(original),review.expectation(original),previous]});
...
for(const item of bank.items){
 ...
 assert.equal(item.source_ref.original_cefr_level,'B2');assert.equal(item.level_review.level,'B1');assert.equal(item.language_function,'formeel_argumentatief');
...
for(const id of ['093','103','113'])assert.match(current.itemById('ZULLEN_B2_'+id).model_answer,/uitwijzen of/);
for(const id of ['094','104','114'])assert.match(current.itemById('ZULLEN_B2_'+id).model_answer,/uitwijzen of extra middelen/);
for(const [id,pattern] of [['096',/helpen beoordelen/],['101',/waarschijnlijk/],['111',/waarschijnlijk/]])assert.match(current.itemById('ZULLEN_B2_'+id).model_answer,pattern);
for(const [id,pattern] of [['091',/u zult als u zal/],['112',/stellige aansporing/],['118',/persoon kan vaker komen/]])assert.match(current.itemById('ZULLEN_B2_'+id).explanation,pattern);
...
assert.equal(current.filterSource({topics:['ZULLEN'],levels:['B1']}).length,270);assert.equal(current.filterSource({levels:['B2']}).length,188);
assert.equal(require('../match-pair-contract.js').audit(set).pairs.length,6);
for(const source of [original,review.previous(original),review.references(original),review.passive(original),review.existence(original),review.appearance(original),review.reporting(original),review.argument(original),review.probability(original),review.expectation(original),previous]){
 const old=createContentRuntime(source,{familyId:'grammar'}),filters={topics:['ZULLEN'],levels:['B2'],language_functions:['formeel_argumentatief']};
 ...
 const saved=old.specFromFilters({...filters,exercise_types:['invullen']}),updated=current.currentSelection(saved);assert.deepEqual(updated.scope_clauses[0].cefr_levels,['B1']);assert.deepEqual(current.selectionPool(updated).map(i=>i.content_item_id).sort(),['ZULLEN_B2_092','ZULLEN_B2_104','ZULLEN_B2_116']);
}
...
console.log('PASS 30 deliberation Zullen tasks: concrete context, B1 advice, no answer leaks, six closed pairs, grammar/meaning repairs, other 1410 unchanged and eleven historical versions restored across five engines.');
