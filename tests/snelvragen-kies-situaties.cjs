...
const source=require('../data/snelvragen-content.js'),direct=require('../data/snelvragen-review.js').revise(source),firstReview=require('../data/snelvragen-vertel-review.js'),first=firstReview.revise(direct),secondReview=require('../data/snelvragen-vertel-vervolg.js'),second=secondReview.revise(first),thirdReview=require('../data/snelvragen-vertel-situaties.js'),third=thirdReview.revise(second),fourthReview=require('../data/snelvragen-vertel-uitleg.js'),fourth=fourthReview.revise(third),fifth=require('../data/snelvragen-vertel-afronding.js').revise(fourth),sixth=require('../data/snelvragen-vragen-review.js').revise(fifth),seventh=require('../data/snelvragen-vragen-vervolg.js').revise(sixth),eighth=require('../data/snelvragen-vragen-situaties.js').revise(seventh),ninth=require('../data/snelvragen-vragen-uitleg.js').revise(eighth),tenth=require('../data/snelvragen-vragen-afronding.js').revise(ninth),eleventh=require('../data/snelvragen-kies-review.js').revise(tenth),previous=require('../data/snelvragen-kies-vervolg.js').revise(eleventh),review=require('../data/snelvragen-kies-situaties.js'),{createContentRuntime}=require('../content-runtime.js'),guidance=require('../content-guidance.js');
...
assert.deepEqual(ids,source.items.filter(i=>i.topic==='quick-choose').slice(240,360).map(i=>i.content_item_id));assert.equal(ids.length,120);assert.equal(JSON.stringify(previous),snapshot);
assert.deepEqual(bank.items.filter(i=>!idSet.has(i.content_item_id)),previous.items.filter(i=>!idSet.has(i.content_item_id)),'2501 other tasks unchanged, including previous 1627 reviewed questions');assert.equal(bank.items.length,2621);
const runtime=createContentRuntime(require('../data/tussen-de-regels.js'));runtime.registerBank(bank,{...meta,previousVersions:[source,direct,first,second,third,fourth,fifth,sixth,seventh,eighth,ninth,tenth,eleventh,previous]});
for(const b of [source,direct,first,second,third,fourth,fifth,sixth,seventh,eighth,ninth,tenth,eleventh,previous])guidance.registerHistorical(b);guidance.register(bank.guidance);guidance.complete(bank.items);
...
assert.equal(models,1);assert.equal(changedModels,0);assert.equal(changedLevels,0);
assert.deepEqual(ids.reduce((a,id)=>(a[runtime.itemById(id).cefr_level]=(a[runtime.itemById(id).cefr_level]||0)+1,a),{}),{A2:120});
for(const oldBank of [source,direct,first,second,third,fourth,fifth,sixth,seventh,eighth,ninth,tenth,eleventh,previous]){
...
assert.match(get('r2-014').context,/vóór vertrek/);assert.match(get('r2-039').prompt,/fiets aan de hand/);assert.match(get('r2-052').context,/leidinggevende.*inpakken verdelen/);assert.match(get('r2-064').context,/stickers/);assert.match(get('r2-081').context,/10.00 uur.*10.30 uur/);assert.match(get('r2-090').context,/Zaterdag is het open.*ligt je pakket er nog/);assert.match(get('r2-098').prompt,/berging/);assert.match(get('r2-119').context,/nemen gevonden spullen aan/);
assert.equal(get('r2-018').practice_group,get('r1-016').practice_group,'Lunch variants share a spread group');assert.equal(get('r2-122').practice_group,get('r2-083').practice_group,'Changing a meeting shares a group');
...
assert.equal(bank.items.filter(i=>i.topic==='quick-choose'&&!i.version.includes('.kies.')).length,253);
assert.throws(()=>review.revise(source));
console.log('PASS third 120 Kies: 120 A2, 1 source model unchanged, 119 without invented models; 2501 other tasks unchanged; 5040 historical restores plus pre-ref sessions, individual guidance, open preferences and three-engine parity.');
