...
(async()=>{
 ...
 await page.locator('[name=topic]').selectOption('KUNNEN');await page.locator('[name=level]').selectOption('A1');await page.locator('.practice-engine:has(input[value=CARDS])').click();
 assert.equal(await page.evaluate(()=>ContentRuntime.filterSource({bank_ids:['CB-GRAM-003']}).length),450);
 ...
 await page.locator('#practiceForm [data-guidance=erk]').click();assert.equal(await page.locator('.guidance-status').textContent(),'A1');await page.keyboard.press('Escape');
 ...
 await page.setViewportSize({width:1440,height:1000});const dir=path.join(root,'tests/artifacts/modals');fs.mkdirSync(dir,{recursive:true});await page.screenshot({path:path.join(dir,'preparation.png')});
 await page.locator('#practiceSave').click();await page.locator('#lessonName').fill('Kunnen oefenen');await page.locator('#lessonSaveForm .primary').click();await page.waitForFunction(()=>!document.querySelector('#gameDialog').open);
 ...
 await page.locator('[data-ghelp]').click();assert.equal(await page.locator('.guidance-status').textContent(),'A1');await page.keyboard.press('Escape');
 await page.evaluate(()=>{ContentUI.clearEditing();const filters={bank_ids:['CB-GRAM-003']},pool=ContentRuntime.filterSource(filters);ContentUI.launch(ContentRuntime.createSession({filters,targetDurationSeconds:pool.reduce((n,i)=>n+i.estimated_duration_seconds,0),selectedGameEngine:'CARDS'}));settingsPatch({reducedMotion:true})});
 const seen=new Set();for(let n=0;n<450;n++){
 ...
 for(const engine of ['BOARD','WHEEL','QUIZ','SEQUENCE']){
  await page.evaluate(engine=>{const filters={bank_ids:['CB-GRAM-003'],levels:['A2'],exercise_types:engine==='SEQUENCE'?['zinnen_leggen']:[]};APP.fixedRoll=1;ContentUI.launch(ContentRuntime.createSession({filters,targetDurationSeconds:300,selectedGameEngine:engine,organizationMode:'groups',seed:11}))},engine);
  await page.locator('[data-ghelp]').click();assert.equal(await page.locator('.guidance-status').textContent(),'A2');await page.keyboard.press('Escape');
  if(engine==='BOARD'){await page.locator('#primaryGame').click();await page.waitForFunction(()=>!boardBusy&&document.querySelector('#taskDrawer.open'));assert.match(await page.locator('#taskDrawer').getAttribute('data-task-id'),/^MOD_/);}
  if(engine==='WHEEL'){await page.locator('#primaryGame').click();await page.waitForFunction(()=>!document.querySelector('#primaryGame').disabled);assert.match(await page.locator('.na-wheel-result [data-content-item-id]').getAttribute('data-content-item-id'),/^MOD_/);await page.screenshot({path:path.join(dir,'wheel.png')});}
 ...
 await page.evaluate(()=>{CONTENT_VERT001.stop();ContentUI.clearEditing();ContentUI.open({topic:'MODAAL_ALLES',level:'B1',engine:'CARDS'});});
 const mix=await page.evaluate(()=>({pool:ContentRuntime.selectionPool(ContentUI.selectionSpec()).map(i=>i.content_item_id),preview:ContentUI.previewItems().map(i=>i.content_item_id)}));assert.ok(mix.pool.some(id=>id.startsWith('ZULLEN_'))&&mix.pool.some(id=>id.startsWith('ZOUDEN_'))&&mix.pool.some(id=>id.startsWith('MOD_')));assert.equal(new Set(mix.pool).size,mix.pool.length);
 await page.locator('#practiceStart').click();assert.deepEqual(await page.evaluate(()=>APP.contentSessionConfig.selected_item_ids),mix.preview);
 assert.deepEqual(errors,[]);console.log('PASS PB003 browser: all 450 cards, five screen sizes, actual lesson, both routes, save/reload, four other engines, guidance and original/new modal mix.');
