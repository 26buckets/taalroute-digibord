...
const runtime = vm.runInNewContext(read('data-bundle.js') + ';window.DIGIBORD_DATA', {window:{}});
...
assert.equal(rebuses.length, 10);
assert.equal(bundled.filter(c => c.visualRebus).length, 10);
...
for (const c of rebuses) {
 ...
 assert.ok(!rendered.includes(c.situation));
...
console.log('PASS: all ten pictures, exact card IDs, original contexts, image-first rendering, solution explanations and JSON/runtime parity.');
