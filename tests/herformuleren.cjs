...
const {bank,originals,review,output}=require('../scripts/import-herformuleren.cjs'),{createContentRuntime}=require('../content-runtime.js'),guidance=require('../content-guidance.js');
assert.equal(fs.readFileSync(require.resolve('../data/herformuleren.js'),'utf8'),output);
...
for(const [n,i] of bank.items.entries()){
 ...
 assert.equal(i.content_item_id,'C1_HF_'+String(n+1).padStart(3,'0'));for(const k of ['title','context','prompt'])assert.equal(i[k],task[k]);assert.deepEqual(i.options,task.options.map((s,n)=>'ABC'[n]+'. '+s));assert.equal(i.model_answer,i.options['ABC'.indexOf(task.letter)]);
...
assert.deepEqual(levels,{B2:27,B1:23,C1:10});assert.deepEqual(counts,{A:20,B:20,C:20});assert.equal(Object.values(review.items).filter(r=>Object.keys(r.patch).length).length,60);
assert.equal(bank.items.filter(i=>i.correct_answer===null).length,20);
for(const [n,i] of bank.items.entries())assert.equal(i.model_answer[0],originals[n].letter,'Reviewed key keeps original option position');
const card=n=>runtime.itemById('C1_HF_'+String(n).padStart(3,'0'));
assert.equal(card(4).model_answer,'B. Minstens één deelnemer was niet op tijd.');
assert.equal(card(13).model_answer,'C. Er zal worden beoordeeld of het voorstel haalbaar is.');
assert.match(card(17).context,/Met dit bedoelt de spreker/);
assert.equal(card(24).model_answer,'C. De directie heeft het voorstel afgewezen.');
assert.ok(!card(32).model_answer.includes('overwegend'));
assert.ok(!card(38).model_answer.includes('meer weerstand'));
assert.match(card(44).model_answer,/kunnen instemmen/);
assert.match(card(49).model_answer,/zou de directie/);
assert.match(card(54).context,/Met daardoor bedoelt/);
assert.match(card(56).model_answer,/alleen voor nieuwe klanten/);
for(const n of [15,20,24,27,29,33,35,39,45,48,50,57,59,60])assert.ok(!/de (eerste|tweede|derde) versie/i.test(card(n).explanation),'No stale option-position explanation '+n);
assert.ok(!bank.items.some(i=>/epistemisch|scope|actor|modaliteit|nominalisatie|ambiguïteit|generaliseerbaarheid/i.test(i.prompt+' '+i.title)));
...
console.log('PASS Herformuleren: 60 source IDs, frozen hash, explicit semantic repairs, B1/B2/C1 23/27/10, old banks unchanged, overlap against 5521 + 50, lesson guidance, both routes and game restrictions.');
