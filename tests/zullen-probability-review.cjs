...
const original=require('../data/content-vert001-er-b1.js'),api=require('../data/grammar-review.js'),review={...api,revise:api.probability,version:api.probabilityVersion},{createContentRuntime}=require('../content-runtime.js');
const previous=review.argument(original),bank=review.revise(original),hash=x=>crypto.createHash('sha256').update(JSON.stringify(x)).digest('hex');
assert.equal(hash(previous.items),'301bb8f6b096dba4346f1c0e45b67abc9de68cba085da03c13282dfe3b2b8f95');assert.equal(hash(bank.items),bank.source_sha256);
const current=createContentRuntime(bank,{familyId:'grammar',previousVersions:[original,review.previous(original),review.references(original),review.passive(original),review.existence(original),review.appearance(original),review.reporting(original),previous]});
...
for(const item of bank.items){
 ...
 assert.equal(item.source_ref.original_cefr_level,'B2');assert.equal(item.level_review.level,'B1');assert.equal(item.language_function,'aanname');
...
for(const [id,pattern] of [['009',/teruggestuurde producten niet in beide totalen/],['021',/overstap op het nieuwe computersysteem/],['020',/kosten zullen/]])assert.match(current.itemById('ZULLEN_B2_'+id).model_answer,pattern);
for(const [id,pattern] of [['003',/besluit over het nieuwe rooster/],['005',/kwamen weinig mensen/],['007',/nu elf uur/],['011',/naar huis vertrokken/],['018',/rente.*verhoogd/],['019',/tien minuten.*naar kantoor/],['030',/naar kantoor vertrokken/]])assert.match(current.itemById('ZULLEN_B2_'+id).context,pattern);
...
assert.equal(current.filterSource({topics:['ZULLEN'],levels:['B1']}).length,180);assert.equal(current.filterSource({levels:['B2']}).length,278);
assert.equal(require('../match-pair-contract.js').audit(set).pairs.length,6);
for(const source of [original,review.previous(original),review.references(original),review.passive(original),review.existence(original),review.appearance(original),review.reporting(original),previous]){
 const old=createContentRuntime(source,{familyId:'grammar'}),filters={topics:['ZULLEN'],levels:['B2'],language_functions:['aanname']};
 ...
 const saved=old.specFromFilters({...filters,exercise_types:['invullen']}),updated=current.currentSelection(saved);assert.deepEqual(updated.scope_clauses[0].cefr_levels,['B1']);assert.deepEqual(current.selectionPool(updated).map(i=>i.content_item_id).sort(),['ZULLEN_B2_002','ZULLEN_B2_014','ZULLEN_B2_026']);
}
...
console.log('PASS 30 probability Zullen tasks: concrete context, B1 advice, no answer leaks, six closed pairs, grammar/meaning repairs, other 1410 unchanged and eight historical versions restored across five engines.');
