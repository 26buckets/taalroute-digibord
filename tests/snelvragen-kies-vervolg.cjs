...
const source=require('../data/snelvragen-content.js'),direct=require('../data/snelvragen-review.js').revise(source),firstReview=require('../data/snelvragen-vertel-review.js'),first=firstReview.revise(direct),secondReview=require('../data/snelvragen-vertel-vervolg.js'),second=secondReview.revise(first),thirdReview=require('../data/snelvragen-vertel-situaties.js'),third=thirdReview.revise(second),fourthReview=require('../data/snelvragen-vertel-uitleg.js'),fourth=fourthReview.revise(third),fifth=require('../data/snelvragen-vertel-afronding.js').revise(fourth),sixth=require('../data/snelvragen-vragen-review.js').revise(fifth),seventh=require('../data/snelvragen-vragen-vervolg.js').revise(sixth),eighth=require('../data/snelvragen-vragen-situaties.js').revise(seventh),ninth=require('../data/snelvragen-vragen-uitleg.js').revise(eighth),tenth=require('../data/snelvragen-vragen-afronding.js').revise(ninth),previous=require('../data/snelvragen-kies-review.js').revise(tenth),review=require('../data/snelvragen-kies-vervolg.js'),{createContentRuntime}=require('../content-runtime.js'),guidance=require('../content-guidance.js');
...
assert.deepEqual(ids,source.items.filter(i=>i.topic==='quick-choose').slice(120,240).map(i=>i.content_item_id));assert.equal(ids.length,120);assert.equal(JSON.stringify(previous),snapshot);
assert.deepEqual(bank.items.filter(i=>!idSet.has(i.content_item_id)),previous.items.filter(i=>!idSet.has(i.content_item_id)),'2501 other tasks unchanged, including previous 1507 reviewed questions');assert.equal(bank.items.length,2621);
const runtime=createContentRuntime(require('../data/tussen-de-regels.js'));runtime.registerBank(bank,{...meta,previousVersions:[source,direct,first,second,third,fourth,fifth,sixth,seventh,eighth,ninth,tenth,previous]});
for(const b of [source,direct,first,second,third,fourth,fifth,sixth,seventh,eighth,ninth,tenth,previous])guidance.registerHistorical(b);guidance.register(bank.guidance);guidance.complete(bank.items);
...
assert.equal(models,23);assert.equal(changedModels,0);assert.equal(changedLevels,0);
assert.deepEqual(ids.reduce((a,id)=>(a[runtime.itemById(id).cefr_level]=(a[runtime.itemById(id).cefr_level]||0)+1,a),{}),{A2:120});
for(const oldBank of [source,direct,first,second,third,fourth,fifth,sixth,seventh,eighth,ninth,tenth,previous]){
...
assert.match(get('r1-014').context,/daglicht/);assert.match(get('r1-036').context,/dichtbij.*droog/);assert.match(get('r1-039').context,/tien minuten.*bus kunt zien/);assert.match(get('r1-044').context,/na elkaar/);assert.match(get('r1-049').context,/verwacht/);assert.match(get('r1-052').context,/bekers.*tafels/);assert.match(get('r1-056').prompt,/elke week een andere activiteit/);assert.match(get('r1-074').context,/bestemming ook opladen/);assert.match(get('r1-089').context,/afhaalpunt/);assert.match(get('r1-120').prompt,/buurman/);assert.match(get('r2-002').prompt,/vertellen wat jullie al hebben gedaan/);
assert.equal(get('r1-081').practice_group,get('r0-085').practice_group,'Silence/music variants spread together across levels');
assert.equal(get('r1-012').practice_group,get('r1-017').practice_group,'Both alone/together cooking choices are spread together');assert.equal(get('r1-016').practice_group,get('r1-095').practice_group,'Both lunch preparation choices are spread together');
...
assert.equal(bank.items.filter(i=>i.topic==='quick-choose'&&!i.version.includes('.kies.')).length,373);
assert.throws(()=>review.revise(source));
console.log('PASS second 120 Kies: 120 A2, 23 source models unchanged, 97 without invented models; 2501 other tasks unchanged; 4680 historical restores plus pre-ref sessions, individual guidance, open preferences and three-engine parity.');
