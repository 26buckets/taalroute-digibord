...
(async()=>{
 ...
 await page.locator('[name=topic]').selectOption('vergadertaal');assert.deepEqual(await page.locator('[name=level] option:not([disabled])').allTextContents(),['B2','C1']);await page.locator('[name=level]').selectOption('B2');
 ...
 await page.setViewportSize({width:1440,height:1000});const dir=path.join(root,'tests/artifacts/vergadertaal');fs.mkdirSync(dir,{recursive:true});await page.screenshot({path:path.join(dir,'preparation.png')});
 await page.locator('#practiceSave').click();await page.locator('#lessonName').fill('Vergaderen');await page.locator('#lessonSaveForm .primary').click();await page.waitForFunction(()=>!document.querySelector('#gameDialog').open);
 ...
 await page.evaluate(()=>{ContentUI.clearEditing();const filters={bank_ids:['CB-MEETING-004']};ContentUI.launch(ContentRuntime.createSession({filters,targetDurationSeconds:ContentRuntime.filterSource(filters).reduce((n,i)=>n+i.estimated_duration_seconds,0),selectedGameEngine:'CARDS'}))});
 ...
 for(const engine of ['BOARD','WHEEL']){
  await page.evaluate(engine=>{APP.fixedRoll=1;settingsPatch({reducedMotion:true});ContentUI.launch(ContentRuntime.createSession({filters:{family_ids:['conversation'],topics:['vergadertaal'],levels:['C1']},targetDurationSeconds:600,selectedGameEngine:engine,seed:11}))},engine);
  ...
  if(engine==='BOARD'){await page.locator('#primaryGame').click();await page.waitForFunction(()=>!boardBusy&&document.querySelector('#taskDrawer.open'));assert.match(await page.locator('#taskDrawer').getAttribute('data-task-id'),/^C1_VB_/);}
  if(engine==='WHEEL'){
   assert.equal(await page.locator('.na-wheel-legend [data-content-item-id]').count(),6);const spun=new Set();
   for(let n=0;n<12;n++){await page.locator('#primaryGame').click();await page.waitForFunction(()=>!document.querySelector('#primaryGame').disabled);const id=await page.locator('.na-wheel-result [data-content-item-id]').getAttribute('data-content-item-id');spun.add(id);assert.match(id,/^C1_VB_/);}
 ...
 assert.deepEqual(errors,[]);console.log('PASS Vergaderen browser: 60 cards, actual lesson and counts, both routes, guidance, suitable games, saving, progress/reload, board/wheel and five screen sizes.');
