...
const original=require('../data/content-vert001-er-b1.js'),api=require('../data/grammar-review.js'),review={...api,revise:api.message,version:api.messageVersion},{createContentRuntime}=require('../content-runtime.js');
const previous=review.past(original),bank=review.revise(original),hash=x=>crypto.createHash('sha256').update(JSON.stringify(x)).digest('hex');
assert.equal(hash(previous.items),'a2acae60fe73cf30a2931397c5d1de929a07767aedfe463d2d80393e8f6d979c');assert.equal(hash(bank.items),bank.source_sha256);
const current=createContentRuntime(bank,{familyId:'grammar',previousVersions:[original,review.previous(original),review.references(original),review.passive(original),review.existence(original),review.appearance(original),review.reporting(original),review.argument(original),review.probability(original),review.expectation(original),review.certainty(original),review.deliberation(original),review.boundary(original),previous]});
...
for(const item of bank.items){
 ...
 assert.equal(item.source_ref.original_cefr_level,'B2');assert.equal(item.level_review.level,'B1');assert.equal(item.language_function,'gerapporteerde_onzekerheid');
...
for(const [id,pattern] of [['038',/Volgens een medewerker/],['041',/Volgens twee medewerkers/],['048',/nog geen akkoord/],['049',/Volgens het nieuwsbericht/],['051',/meerdere organisaties/],['053',/mogelijk worden uitgesteld/],['060',/Volgens een collega/]])assert.match(current.itemById('ZOUDEN_B2_'+id).model_answer,pattern);
assert.doesNotMatch(current.itemById('ZOUDEN_B2_048').model_answer,/definitief/);
assert.match(current.itemById('ZOUDEN_B2_052').explanation,/niet dat er niets/);
assert.equal(current.filterSource({topics:['ZOUDEN'],levels:['B1']}).length,180);
...
assert.equal(current.filterSource({topics:['ZOUDEN'],levels:['B2']}).length,120);assert.equal(current.filterSource({levels:['B2']}).length,128);
...
for(const source of [original,review.previous(original),review.references(original),review.passive(original),review.existence(original),review.appearance(original),review.reporting(original),review.argument(original),review.probability(original),review.expectation(original),review.certainty(original),review.deliberation(original),review.boundary(original),previous]){
 const old=createContentRuntime(source,{familyId:'grammar'}),filters={topics:['ZOUDEN'],levels:['B2'],language_functions:['gerapporteerde_onzekerheid']};
 ...
 const saved=old.specFromFilters({...filters,exercise_types:['invullen']}),updated=current.currentSelection(saved);assert.deepEqual(updated.scope_clauses[0].cefr_levels,['B1']);assert.deepEqual(current.selectionPool(updated).map(i=>i.content_item_id).sort(),['ZOUDEN_B2_032','ZOUDEN_B2_044','ZOUDEN_B2_056']);
}
...
console.log('PASS 30 reported-message Zouden tasks: concrete context, B1 advice, no answer leaks, six closed pairs, grammar/meaning repairs, other 1410 unchanged and fourteen historical versions restored across five engines.');
