...
const {bank,originals,review,output}=require('../scripts/import-impliciete-boodschap.cjs'),{createContentRuntime}=require('../content-runtime.js'),guidance=require('../content-guidance.js');
assert.equal(fs.readFileSync(require.resolve('../data/impliciete-boodschap.js'),'utf8'),output);
...
for(const [n,i] of bank.items.entries()){
 ...
 assert.equal(i.content_item_id,'C1_IMB_'+String(n+1).padStart(3,'0'));for(const k of ['title','context','prompt'])assert.equal(i[k],task[k]);assert.deepEqual(i.options,task.options.map((s,n)=>'ABC'[n]+'. '+s));assert.equal(i.model_answer,i.options['ABC'.indexOf(source.letter)]);
...
assert.deepEqual(levels,{B1:9,B2:38,C1:13});assert.deepEqual(counts,{A:20,B:20,C:20});assert.equal(Object.values(review.items).filter(r=>Object.keys(r.patch).length).length,19);
assert.ok(!runtime.itemById('C1_IMB_008').model_answer.includes('geschikter'));assert.match(runtime.itemById('C1_IMB_055').explanation,/beleefde afsluiting/);assert.ok(!runtime.itemById('C1_IMB_046').model_answer.includes('bewust'));
assert.equal(bank.items.filter(i=>i.correct_answer===null).length,31);
...
const previousBanks=[old.bank,workplace.bank,require('../data/vergadertaal.js'),require('../data/content-vert001-er-b1.js'),require('../data/gram-pb003.js'),require('../data/gram-pb002.js'),require('../data/wz-pb003.js'),require('../data/wz-pb004.js'),require('../data/wz-pb005.js'),adapt(require('../Lessen/woorden-zinnen.json')),adapt(require('../data/wz-pb002.js')),require('../data/connections-pilot.js'),adapt.riddles(activityContext.window.DIGIBORD_ACTIVITIES)];
assert.equal(previousBanks.reduce((n,b)=>n+b.items.length,0),5331);
...
console.log('PASS Impliciete boodschap: 60 source IDs, frozen hash, 19 corrections, B1/B2/C1 9/38/13, old banks unchanged, overlap against 5331 + 50, lesson guidance, both routes and game restrictions.');
