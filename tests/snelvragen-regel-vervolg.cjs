...
const source=require('../data/snelvragen-content.js'),review=require('../data/snelvragen-regel-vervolg.js'),guidance=require('../content-guidance.js'),{createContentRuntime}=require('../content-runtime.js');
const historical=[source];
for(const file of ['review','vertel-review','vertel-vervolg','vertel-situaties','vertel-uitleg','vertel-afronding','vragen-review','vragen-vervolg','vragen-situaties','vragen-uitleg','vragen-afronding','kies-review','kies-vervolg','kies-situaties','kies-uitleg','kies-afronding','regel-review'])historical.push(require('../data/snelvragen-'+file+'.js').revise(historical.at(-1)));
...
assert.equal(historical.length,18);assert.equal(JSON.stringify(previous),snapshot);assert.deepEqual(ids,source.items.filter(i=>i.topic==='quick-arrange').slice(120,240).map(i=>i.content_item_id));assert.equal(ids.length,120);assert.equal(bank.items.length,2621);
assert.deepEqual(bank.items.filter(i=>!idSet.has(i.content_item_id)),previous.items.filter(i=>!idSet.has(i.content_item_id)),'Other 2501 tasks, including 2120 reviewed, unchanged');
...
assert.equal(models,24);assert.equal(changedModels,0);
...
assert.match(get('r1-039').context,/tien minuten/);assert.match(get('r1-051').context,/voorraad tellen/);assert.match(get('r1-087').context,/donderdag/);assert.match(get('r1-090').context,/huisgenoot/);assert.match(get('r1-113').context,/Utrecht/);assert.match(get('r1-119').context,/plank/);
assert.equal(ids.filter(id=>source.items.find(i=>i.content_item_id===id).cefr_level!=='A1').length,27);
...
assert.equal(bank.items.filter(i=>i.topic==='quick-arrange'&&!i.version.includes('.regel.')).length,381);assert.throws(()=>review.revise(source));
console.log('PASS second 120 Regel iets: A1 with given support, 27 revised levels, 24 unchanged models, 96 without invented models; 2501 other tasks unchanged; 6480 historical restores plus pre-ref sessions, four lesson links and three-engine parity.');
