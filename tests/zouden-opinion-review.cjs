...
const original=require('../data/content-vert001-er-b1.js'),api=require('../data/grammar-review.js'),review={...api,revise:api.opinion,version:api.opinionVersion},{createContentRuntime}=require('../content-runtime.js');
const previous=review.inference(original),bank=review.revise(original),hash=x=>crypto.createHash('sha256').update(JSON.stringify(x)).digest('hex');
assert.equal(hash(previous.items),'2cbb8f36aa8901d02b2bef07ed06fcc4e9b98e36296721edca095d02c09e719c');assert.equal(hash(bank.items),bank.source_sha256);
...
for(const item of bank.items){
 ...
 assert.equal(item.source_ref.original_cefr_level,'B2');assert.equal(item.level_review.level,'B1');assert.equal(item.language_function,'diplomatiek');
...
for(const [id,pattern] of [['096',/beschrijving van wat er precies misging/],['099',/omdat ik denk/],['101',/reacties van meer klanten/],['108',/waarop de commissie/],['109',/hoeveel computerervaring nodig is/],['111',/niet evenveel lessen/]])assert.match(current.itemById('ZOUDEN_B2_'+id).model_answer,pattern);
assert.doesNotMatch(current.itemById('ZOUDEN_B2_111').model_answer,/informatie ontbreekt/);
assert.match(current.itemById('ZOUDEN_B2_118').explanation,/niet ineens onzeker/);
for(const id of ['094','106','118'])assert.doesNotMatch(current.itemById('ZOUDEN_B2_'+id).prompt,/zal willen/);
assert.equal(current.filterSource({topics:['ZOUDEN'],levels:['B1']}).length,240);
...
assert.equal(current.filterSource({topics:['ZOUDEN'],levels:['B2']}).length,60);assert.equal(current.filterSource({levels:['B2']}).length,68);
...
for(const source of [original,review.previous(original),review.references(original),review.passive(original),review.existence(original),review.appearance(original),review.reporting(original),review.argument(original),review.probability(original),review.expectation(original),review.certainty(original),review.deliberation(original),review.boundary(original),review.past(original),review.message(original),previous]){
 const old=createContentRuntime(source,{familyId:'grammar'}),filters={topics:['ZOUDEN'],levels:['B2'],language_functions:['diplomatiek']};
 ...
 const saved=old.specFromFilters({...filters,exercise_types:['invullen']}),updated=current.currentSelection(saved);assert.deepEqual(updated.scope_clauses[0].cefr_levels,['B1']);assert.deepEqual(current.selectionPool(updated).map(i=>i.content_item_id).sort(),['ZOUDEN_B2_092','ZOUDEN_B2_104','ZOUDEN_B2_116']);
}
...
console.log('PASS 30 cautious-opinion Zouden tasks: concrete context, B1 advice, no answer leaks, three closed pairs, grammar/meaning repairs, other 1410 unchanged and sixteen historical versions restored across five engines.');
