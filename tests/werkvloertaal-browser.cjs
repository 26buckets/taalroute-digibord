...
(async()=>{
 ...
 await page.locator('[name=topic]').selectOption('werkvloertaal');await page.locator('[name=level]').selectOption('B2');
 ...
 await page.setViewportSize({width:1440,height:1000});const dir=path.join(root,'tests/artifacts/werkvloertaal');fs.mkdirSync(dir,{recursive:true});await page.screenshot({path:path.join(dir,'preparation.png')});
 await page.locator('#practiceSave').click();await page.locator('#lessonName').fill('Werkvloertaal');await page.locator('#lessonSaveForm .primary').click();await page.waitForFunction(()=>!document.querySelector('#gameDialog').open);
 ...
 await page.evaluate(()=>{ContentUI.clearEditing();const filters={bank_ids:['CB-WORK-003']};ContentUI.launch(ContentRuntime.createSession({filters,targetDurationSeconds:ContentRuntime.filterSource(filters).reduce((n,i)=>n+i.estimated_duration_seconds,0),selectedGameEngine:'CARDS'}))});
 const seen=new Set();for(let n=0;n<100;n++){
 ...
 for(const engine of ['BOARD','WHEEL']){
  await page.evaluate(engine=>{APP.fixedRoll=1;settingsPatch({reducedMotion:true});ContentUI.launch(ContentRuntime.createSession({filters:{family_ids:['conversation'],topics:['werkvloertaal'],levels:['C1']},targetDurationSeconds:600,selectedGameEngine:engine,seed:11}))},engine);
  ...
  if(engine==='BOARD'){await page.locator('#primaryGame').click();await page.waitForFunction(()=>!boardBusy&&document.querySelector('#taskDrawer.open'));assert.match(await page.locator('#taskDrawer').getAttribute('data-task-id'),/^C1_WK_/);}
  if(engine==='WHEEL'){
   assert.equal(await page.locator('.na-wheel-legend [data-content-item-id]').count(),6);const spun=new Set();
   for(let n=0;n<12;n++){await page.locator('#primaryGame').click();await page.waitForFunction(()=>!document.querySelector('#primaryGame').disabled);const id=await page.locator('.na-wheel-result [data-content-item-id]').getAttribute('data-content-item-id');spun.add(id);assert.match(id,/^C1_WK_/);}
 ...
 assert.deepEqual(errors,[]);console.log('PASS Werkvloertaal browser: 100 cards, actual lesson and counts, both routes, guidance, suitable games, saving, progress/reload, board/wheel and five screen sizes.');
