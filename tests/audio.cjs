...
(async()=>{await new Promise(r=>server.listen(0,'127.0.0.1',r));const b=await chromium.launch({executablePath:process.env.CHROME_PATH,headless:true});try{
 ...
 await control(p,'#pp-sound','click');await p.reload();assert.equal(await p.locator('#pp-sound').getAttribute('aria-pressed'),'false');
 ...
 assert(await p.locator('.aw-utilities #pp-sound').isVisible());await control(p,'#pp-sound','click');await p.waitForFunction(()=>__played===1);await p.waitForFunction(()=>__audio[0].ended);
 await p.getByRole('button',{name:'Werp alle negen',exact:true}).click();await p.waitForFunction(()=>__played===2);await control(p,'#pp-sound','click');assert(await p.evaluate(()=>__audio[0].paused));
