...
(async()=>{
 ...
 const first=await page.evaluate(()=>{
  ...
  const service=ContentStorage.createContentStorageService({
   ...
   referenceResolvers:{game_variant:refId=>['rotterdam','zwolle','draaiwiel','content-pb001'].includes(refId)}
  });
 ...
 const second=await page.evaluate(ids=>{
  const adapter=ContentStorage.createLocalStorageAdapter(localStorage,{key:'content002b-browser-test'});
  const service=ContentStorage.createContentStorageService({adapter,contentRuntime:ContentRuntime,progressAdapters:{BOARD:{canResume:p=>p.progress_schema_version==='BOARD-PROGRESS-1'}},referenceResolvers:{game_variant:refId=>['rotterdam','zwolle','draaiwiel','content-pb001'].includes(refId)}});
