...
(async()=>{
 ...
 assert.equal(await p.locator('#questionMode').count(),1);
 assert.equal(await p.locator('#modeSelect').count(),1);
 ...
 await p.getByRole('button',{name:'Uitleg: Spelvorm',exact:true}).hover();
 ...
 assert.ok(await p.locator('#boardOptions').evaluate(el=>el.scrollWidth<=el.clientWidth+1),'compact options do not overflow horizontally');
