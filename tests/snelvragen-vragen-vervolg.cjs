...
assert.deepEqual(ids,source.items.filter(i=>i.topic==='quick-ask').slice(120,240).map(i=>i.content_item_id));assert.equal(ids.length,120);assert.equal(JSON.stringify(previous),snapshot);
assert.deepEqual(bank.items.filter(i=>!idSet.has(i.content_item_id)),previous.items.filter(i=>!idSet.has(i.content_item_id)),'2501 other tasks unchanged, including previous 941 reviewed questions');assert.equal(bank.items.length,2621);
const runtime=createContentRuntime(require('../data/tussen-de-regels.js'));runtime.registerBank(bank,{...meta,previousVersions:[source,direct,first,second,third,fourth,fifth,previous]});
for(const b of [source,direct,first,second,third,fourth,fifth,previous])guidance.registerHistorical(b);guidance.register(bank.guidance);guidance.complete(bank.items);
...
assert.equal(models,24);assert.equal(changedModels,0);assert.equal(changedLevels,103);
assert.deepEqual(ids.reduce((a,id)=>(a[runtime.itemById(id).cefr_level]=(a[runtime.itemById(id).cefr_level]||0)+1,a),{}),{A1:103,A2:17});
for(const oldBank of [source,direct,first,second,third,fourth,fifth,previous]){
...
const get=n=>runtime.itemById('sq-r1-square-'+n);
assert.match(get('028').context,/verpakking koekjes/);assert.match(get('051').context,/dozen inpakken/);assert.match(get('062').context,/trap.*lift/);assert.match(get('062').prompt,/zo'n ingang/);assert.match(get('070').explanation,/geen toestemming/);assert.match(get('075').context,/tien uur/);assert.match(get('089').context,/bezorgdienst meldt.*bij een buur/i);assert.match(get('095').context,/vijftig euro/);assert.match(get('099').context,/gaat schoonmaken/);assert.match(get('121').context,/morgen/);
assert.equal(get('121').model_answer,'Hoe laat sta je morgen op?');
...
console.log('PASS next 120 Stel een vraag: 103 A1/17 A2, 103 level repairs, 24 original models unchanged; 2501 other tasks unchanged; 2880 exact historical restores plus pre-ref sessions, individual guidance and three-engine parity.');
