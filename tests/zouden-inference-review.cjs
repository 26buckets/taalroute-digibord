...
const original=require('../data/content-vert001-er-b1.js'),api=require('../data/grammar-review.js'),review={...api,revise:api.inference,version:api.inferenceVersion},{createContentRuntime}=require('../content-runtime.js');
const previous=review.message(original),bank=review.revise(original),hash=x=>crypto.createHash('sha256').update(JSON.stringify(x)).digest('hex');
assert.equal(hash(previous.items),'bfecd1425b64eb0ceb5e6d934f9aa318bf7120932d87cd20cda0858f794f09ec');assert.equal(hash(bank.items),bank.source_sha256);
...
for(const item of bank.items){
 ...
 assert.equal(item.source_ref.original_cefr_level,'B2');assert.equal(item.level_review.level,'B1');assert.equal(item.language_function,'voorzichtige_inferentie');
...
for(const [id,pattern] of [['067',/klanten van deze winkel/],['069',/geheel of gedeeltelijk/],['071',/onzekerheid over de toekomstige vraag/],['072',/erop kunnen wijzen/],['078',/eisen van de test/],['083',/sommige deelnemers/],['090',/zou kunnen helpen/]])assert.match(current.itemById('ZOUDEN_B2_'+id).model_answer,pattern);
assert.doesNotMatch(current.itemById('ZOUDEN_B2_072').model_answer,/aantonen/);
assert.match(current.itemById('ZOUDEN_B2_080').context,/enkele deelnemers gestopt/);
assert.doesNotMatch(current.itemById('ZOUDEN_B2_090').model_answer,/noodzakelijk/);
assert.equal(current.filterSource({topics:['ZOUDEN'],levels:['B1']}).length,210);
...
assert.equal(current.filterSource({topics:['ZOUDEN'],levels:['B2']}).length,90);assert.equal(current.filterSource({levels:['B2']}).length,98);
assert.equal(require('../match-pair-contract.js').audit(set).pairs.length,3);
for(const source of [original,review.previous(original),review.references(original),review.passive(original),review.existence(original),review.appearance(original),review.reporting(original),review.argument(original),review.probability(original),review.expectation(original),review.certainty(original),review.deliberation(original),review.boundary(original),review.past(original),previous]){
 const old=createContentRuntime(source,{familyId:'grammar'}),filters={topics:['ZOUDEN'],levels:['B2'],language_functions:['voorzichtige_inferentie']};
 ...
 const saved=old.specFromFilters({...filters,exercise_types:['invullen']}),updated=current.currentSelection(saved);assert.deepEqual(updated.scope_clauses[0].cefr_levels,['B1']);assert.deepEqual(current.selectionPool(updated).map(i=>i.content_item_id).sort(),['ZOUDEN_B2_062','ZOUDEN_B2_074','ZOUDEN_B2_086']);
}
...
console.log('PASS 30 possible-explanation Zouden tasks: concrete context, B1 advice, no answer leaks, three closed pairs, grammar/meaning repairs, other 1410 unchanged and fifteen historical versions restored across five engines.');
