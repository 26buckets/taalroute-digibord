...
for(const engine of ['BOARD','WHEEL','CARDS','DICE','QUIZ']){
...
assert.equal(runtime.eligibleItems(['BOARD','WHEEL','CARDS','DICE','QUIZ']).length,1440);
...
for(const topic of ['ER','ZULLEN','ZOUDEN']){
 for(const level of ['A2','B1','B2']){
  const session=runtime.createSession({seed:77,targetDurationSeconds:300,filters:{topics:[topic],levels:[level]},selectionTopic:topic,engines:['BOARD','WHEEL','CARDS','DICE','QUIZ'],organizationMode:'groups'});
  ...
  const board=runtime.enginePool('BOARD',session),wheel=runtime.enginePool('WHEEL',session),cards=runtime.enginePool('CARDS',session),dice=runtime.enginePool('DICE',session),quiz=runtime.enginePool('QUIZ',session);
  ...
  assert.deepEqual(quiz.map(x=>x.content_item_id),session.selected_item_ids);
  for(let i=0;i<board.length;i++){assert.strictEqual(board[i],wheel[i]);assert.strictEqual(board[i],cards[i]);assert.strictEqual(board[i],dice[i]);assert.strictEqual(board[i],quiz[i])}
 }
}
const modal=runtime.createSession({seed:88,targetDurationSeconds:600,filters:{topics:['ZULLEN','ZOUDEN'],levels:['B1'],family_tags:['MODAAL']},selectionTopic:'MODAAL',engines:['BOARD','WHEEL','CARDS','DICE','QUIZ'],organizationMode:'groups'});
...
const repeat=runtime.createSession({seed:88,targetDurationSeconds:600,filters:{topics:['ZULLEN','ZOUDEN'],levels:['B1'],family_tags:['MODAAL']},selectionTopic:'MODAAL',engines:['BOARD','WHEEL','CARDS','DICE','QUIZ'],organizationMode:'groups'});
assert.deepEqual(repeat.selected_item_ids,modal.selected_item_ids);
const other=runtime.createSession({seed:89,targetDurationSeconds:600,filters:{topics:['ZULLEN','ZOUDEN'],levels:['B1'],family_tags:['MODAAL']},selectionTopic:'MODAAL',engines:['BOARD','WHEEL','CARDS','DICE','QUIZ'],organizationMode:'groups'});
...
console.log('PASS: CONTENT 000 full PB001 runtime, all 1440 records, A2-B2, ER/ZULLEN/ZOUDEN/MODAAL, ORDER adapter and shared BOARD/WHEEL/CARDS/DICE/QUIZ SessionConfig.');
