...
for(const engine of ['BOARD','WHEEL','CARDS']){
...
assert.equal(runtime.eligibleItems(['BOARD','WHEEL','CARDS']).length,1440);
...
for(const topic of ['ER','ZULLEN','ZOUDEN']){
 for(const level of ['A2','B1','B2']){
  const session=runtime.createSession({seed:77,targetDurationSeconds:300,filters:{topics:[topic],levels:[level]},selectionTopic:topic,engines:['BOARD','WHEEL','CARDS']});
  ...
  for(let i=0;i<board.length;i++){assert.strictEqual(board[i],wheel[i]);assert.strictEqual(board[i],cards[i])}
 }
}
const modal=runtime.createSession({seed:88,targetDurationSeconds:600,filters:{topics:['ZULLEN','ZOUDEN'],levels:['B1'],family_tags:['MODAAL']},selectionTopic:'MODAAL',engines:['BOARD','WHEEL','CARDS']});
...
const repeat=runtime.createSession({seed:88,targetDurationSeconds:600,filters:{topics:['ZULLEN','ZOUDEN'],levels:['B1'],family_tags:['MODAAL']},selectionTopic:'MODAAL',engines:['BOARD','WHEEL','CARDS']});
assert.deepEqual(repeat.selected_item_ids,modal.selected_item_ids);
const other=runtime.createSession({seed:89,targetDurationSeconds:600,filters:{topics:['ZULLEN','ZOUDEN'],levels:['B1'],family_tags:['MODAAL']},selectionTopic:'MODAAL',engines:['BOARD','WHEEL','CARDS']});
...
console.log('PASS: CONTENT 000 full PB001 runtime, all 1440 records, A2-B2, ER/ZULLEN/ZOUDEN/MODAAL, ORDER adapter and shared BOARD/WHEEL/CARDS SessionConfig.');
