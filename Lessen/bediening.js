globalThis.DigiBoardSupport=(()=>{
 ...
 function show(kind,t,dialog,close){
  ...
  if(kind!=='teacher'){
   const example=kind==='example',hint=kind==='hint',body=example?card('Zo kun je het zeggen',t.model||t.help?.replace(/^Voorbeeld: /,''),true):hint?card('Een begin',t.support||'Probeer een korte zin.',true):card('Doe nu dit',t.variants.extra.instruction,true);
   dialog((example?'Voorbeeld':hint?'Hulp':'Extra stap')+' · '+t.title,'<div class="db-support">'+body+back+'</div>',()=>document.getElementById('db-support-back').onclick=close);return;
  }
 ...
 function organize(){
  ...
  const toolbar=node('div','db-task-support');const hint=node('button','','Hulp');hint.type='button';hint.className='pp-secondary';hint.id='db-matrix-hint';hint.onclick=()=>globalThis.DigiBoardMatrixUI?.hint();toolbar.append(hint);const next=node('button','','Andere opdracht');next.id='db-matrix-next';next.type='button';next.className='pp-secondary';next.onclick=()=>globalThis.DigiBoardMatrixUI?.next();toolbar.append(next);toolbar.setAttribute('role','group');toolbar.setAttribute('aria-label','Bij deze opdracht');
